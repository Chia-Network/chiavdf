...
int_type add_carry(int_type a, int_type b, int carry_in, int &carry_out) {
...
template <class int_type>
int_type sub_carry(int_type a, int_type b, int carry_in, int &carry_out) {
  assert(carry_in == 0 || carry_in == 1);
  uint128 res = uint128(a) - uint128(b) - uint128(carry_in);
...
template <class int_type> int clz(int_type a) {
  ...
  if (a == 0) {
    ...
    return (sizeof(int_type) == 4) ? __builtin_clz(uint32(a))
                                   : __builtin_clzll(uint64(a));
  }
}
...
// all "=" operators truncate ; all operators that return a separate result will
// pad the result as necessary
template <class type, int size> struct fixed_integer {
  ...
  USED integer to_integer() const { return integer(*this); }

  // truncation
  template <int t_size>
  explicit fixed_integer(const fixed_integer<type, t_size> &t) {
    for (int x = 0; x < size + 1; ++x) {
      data[x] = (x < t_size + 1) ? t.data[x] : 0;
    }
  }

  fixed_integer &operator=(const integer &v) {
    return *this = fixed_integer(v);
  }
  template <int t_size>
  fixed_integer &operator=(const fixed_integer<type, t_size> &t) {
    return *this = fixed_integer(t);
  }

  bool is_negative() const { return !is_zero() && data[0] == negative_sign; }
  ...
  static int compare(const type *a, int size_a, type sign_a, const type *b,
                     int size_b, type sign_b) {
    int carry = 0;
    type zero = 0;
    ...
    // opposite signs: use res if 0, otherwise 1 if sign_a is positive, -1 if
    // sign_a is negative
    ...
    // this is used to implement exactly one comparison with a binary result, so
    // that should get rid of all of these branches
    if (sign_a == sign_b) {
  ...
  template <int b_size>
  int compare(const fixed_integer<type, b_size> &b) const {
    return compare(data + 1, size, data[0], b.data + 1, size, b.data[0]);
  }

  // a, b, and res can alias with each other but only if the pointers are equal
  // the sign is not present in a/b/res
  static void add(const type *a, int size_a, type sign_a, const type *b,
                  int size_b, type sign_b, type *res, int size_res,
                  type &sign_res) {
    if (size_b > size_a) {
      swap(a, b);
      swap(size_a, size_b);
    ...
    type mask = sign_a ^ sign_b; // all 1s if opposite signs, else all 0s. this
                                 // isn't affected by swapping
    ...
    int carry;
    add_carry(mask, type(1), 0,
              carry); // carry set if opposite signs, else cleared
    ...
    for (int x = 0; x < size_res; ++x) {
      ...
      // this calculates a-b if they had opposite signs, or a+b if they had the
      // same sign
    ...
    // however, if a subtraction was done and a and b were swapped, then this
    // should be negated
    sign_res = sign_a ^ (swap_mask & mask);

    // todo //figure out how often this happens
    // a subtraction was done and there was a carry out. since the subtraction
    // is unsigned, this means it was done in the wrong order this almost never
    // happens if the numbers are random and don't have excessive padding the
    // subtraction was done in the wrong order if the result is negative the
    // result is negative if each input were padded with 0, and the result limb
    // was ~0 instead of 0 the result limb is: add_carry(0, mask, carry, carry);
  ...
  void operator+=(const fixed_integer &b) {
    add(data + 1, size, data[0], b.data + 1, size, b.data[0], data + 1, size,
        data[0]);
  }

  void operator-=(const fixed_integer &b) {
    add(data + 1, size, data[0], b.data + 1, size, negative_sign ^ b.data[0],
        data + 1, size, data[0]);
  }

  template <int b_size>
  fixed_integer<type, max_constexpr(size, b_size) + 1>
  operator+(const fixed_integer<type, b_size> &b) const {
    const int output_size = max_constexpr(size, b_size) + 1;

    fixed_integer<type, output_size> res;

    add(data + 1, size, data[0], b.data + 1, b_size, b.data[0], res.data + 1,
        output_size, res.data[0]);
  ...
  operator-(const fixed_integer<type, b_size> &b) const {
    ...
    add(data + 1, size, data[0], b.data + 1, b_size, negative_sign ^ b.data[0],
  ...
  static void mad(const type *a, int size_a, type b, const type *c, int size_c,
  ...
  // can't overflow
  // two of these can implement a 1024x512 mul. for 1024x1024, need to do 2x
  // 1024x512 in separate buffers then add them
  static void mad_8x8(array<type, 8> a, array<type, 8> b, array<type, 8> c,
                      array<type, 16> &res) {
    for (int x = 0; x < 8; ++x) {
      res[x] = c[x];
    }
    ...
    for (int x = 0; x < 8; ++x) {
      ...
      //-the lowest register is finished after the first addition in mad. the
      //this_low,previous_high addition is skipped -the highest register does
      //not need to be loaded until the last multiplication in mad. actually
      //this would always load 0
      //  so it is not done
      ...
      // address registers:
      //-will just use a static 32-bit address space for most of the code. can
      //store the stack pointer there then -address registers are only used for
      //b and res if the addresses are not static -the addresses are only used
      //at the end of the loop, so there are spare registers to load the address
      //registers from static
  ...
  void operator*=(type v) {
    mad(data + 1, size, v, nullptr, size, data + 1, size);
  }

  template <int t_size, int this_size>
  static fixed_integer<type, t_size>
  subset(const fixed_integer<type, this_size> &this_v, int start) {
  ...
  void operator<<=(int amount) {
    if (amount == 0) {
      // not sure if intel works with the "previous>>64" statement. might wrap
      // around
      return;
    }
    ...
    for (int x = size - 1; x >= 0; --x) {
      ...
      (*this)[x] =
          ((*this)[x] << amount) | (previous >> (bits_per_limb - amount));
    }
  }
  ...
  template <int b_size>
  fixed_integer<type, size + b_size>
  operator*(const fixed_integer<type, b_size> &b) const {
    ...
    for (int x = 0; x < b_size; ++x) {
      ...
      // todo //figure out why this doesn't work. might have something to do
      // with the msb being set?
      res += r; // unsigned
      /*auto res3=res;
      res3+=r;
  ...
  // this rounds to 0 so it is different from division unless the input is
  // divisible by 2^num
...
fixed_integer<type, size> abs(fixed_integer<type, size> v) {
  v.set_negative(false);
  return v;
}

template <int size>
fixed_integer<uint64, (size + 1) / 2> to_uint64(fixed_integer<uint32, size> v) {
  fixed_integer<uint64, (size + 1) / 2> res;
  res.set_negative(
      v.is_negative()); // sign extend data[0]. can just make data[0] 64 bits if
...
template <int size>
fixed_integer<uint32, size * 2> to_uint32(fixed_integer<uint64, size> v) {
