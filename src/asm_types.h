...
-for scalar registers: "_32" is used for "eax/r8d/etc", "_16" is used for
"ax/r8w/etc", "_8" is used for "al/r8b/etc" -writing to a 32 bit register
zero-extends the result to 64 bits. writing to a 8/16 bit register does not zero
extend
***/
...
struct expand_macros {
  ...
  int next_label_id = 0;
  int next_error_label_id =
      1; // can't be 0 since the id is used as the return code
  ...
  void end_scope() {
    ...
    for (pair<const string, string> &n : scopes.back().name_to_value) {
      bool erase_res = value_to_name.at(n.second).erase(
          make_pair(scopes.size() - 1, n.first));
      assert(erase_res);
    }
    scopes.pop_back();
  }
  ...
  void bind_impl(string name, string value) {
    ...
    bool emplace_res_1 =
        scopes.back().name_to_value.emplace(name, value).second;
    assert(emplace_res_1);

    bool emplace_res_2 =
        value_to_name[value].emplace(scopes.size() - 1, name).second;
  ...
  void append(string s, int line, string file, string func) {
    ...
    if (add_comment) {
      res_text.back().push_back(" # " + scopes.back().scope_name + ":" +
                                to_string(line) + "    ");
  ...
  template <class type> struct void_box { typedef void value; };

  template <class type>
  typename void_box<typename type::value_type>::value bind(const type &a,
                                                           string n) {
    int x = 0;
    for (const auto &c : a) {
...
struct expand_macros_tag {
  ...
  ~expand_macros_tag() { m.tag_stack.pop_back(); }
};

struct expand_macros_scope {
  expand_macros &m;

  expand_macros_scope(expand_macros &t_m, string name, bool is_public = false)
      : m(t_m) {
    m.begin_scope(name, is_public);
  }

  ~expand_macros_scope() { m.end_scope(); }
};

#define EXPAND_MACROS_SCOPE expand_macros_scope c_scope(m, __func__)
#define EXPAND_MACROS_SCOPE_PUBLIC                                             \
  expand_macros_scope c_scope(m, __func__, true)
...
struct reg_vector {
  ...
  reg_vector() {}
  explicit reg_vector(int i, int t_default_num_bits)
      : value(i), default_num_bits(t_default_num_bits) {}
  ...
  string name(int num_bits = 512) const {
    ...
    if (value >= 32 ||
        (!enable_all_instructions && (value >= 16 || num_bits != 128))) {
...
struct reg_spill {
  ...
  reg_spill() {}
  reg_spill(int t_value, int t_size, int t_alignment)
      : value(t_value), size(t_size), alignment(t_alignment) {}

  int get_rsp_offset() const { return value - spill_bytes; }

  // this is negative
  uint64 get_rsp_offset_uint64() const { return uint64(value - spill_bytes); }
...
struct reg_alloc {
  ...
  reg_scalar get_scalar(reg_scalar t_reg = reg_scalar()) {
    ...
    int res = (t_reg.value == -1) ? *scalars.begin()
                                  : scalar_to_order.at(t_reg.value);
  ...
  reg_scalar bind_scalar(expand_macros &m, string name,
                         reg_scalar t_reg = reg_scalar()) {
  ...
  reg_spill bind_spill(expand_macros &m, string name, int size = 8,
                       int alignment = -1) {
