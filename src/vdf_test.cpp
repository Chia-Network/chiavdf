...
const int repeated_square_checkpoint_interval = 1
                                                << 10; // should be a power of 2
#endif
...
// it will generate an output, do a mfence, then increment the sequence number
// non-atomically (since it is the only writer) it can also wait for another
// thread's outputs by spinning on its sequence number (with a timeout) error
// states: -any thread can change its sequence number to "error" which is the
//highest uint64 value -it will do this if any operation fails or if it spins
//too long waiting for another thread's output -also, the spin loop will error
//out if the other thread's sequence number is "error". this will make the
//spinning thread's sequence
//  number also be "error"
//-once a thread has become "error", it will exit the code. the slave threads
//will wait on the barrier and the main thread will just
//  exit the squaring function with a "false" output
//-the error state is the global sequence number with the msb set. this allows
//the sequence number to not be reset across calls will just make every state
// have a 48 bit global sequence number (enough for 22 years) plus a 16 bit
// local sequence number -the last local sequence number is the error state
//-there is no finish state since each state update will change the sequence
//number to a new, unique sequence number to start the squaring, the main thread
// will output A and B then increase its sequence number to the next global
// sequence number -slave threads will wait on this when they are done squaring
//or have outputted the error state -is is assumed that the main thread
//synchronizes with each slave thread to consume its output can probably write
// the synchronization code in c++ then because of how simple it is this is
// trivial to implement and should be reliable if the gcd generates too many
// matricies (more than 32 or so), it should generate an error need to write
// each output to a separate cache line will use the slave core for: cofactors
// for both gcds, calculate C at the start of the squaring, calculate (-v2*c)%a
// as the v2
//  cofactor is being generated. this will use <0,-c> as the initial state
//  instead of <0,1> and will also reduce everything modulo a after each matrix
//  multiplication. it will also calculate C first.
// the slave core will then calculate the partial gcd and the master core will
// calculate the cofactors once the master core has calculated all of the
// cofactors, it will also know the final values of a_copy and k_copy from the
//  slave core. the slave core is done now
// the master core will calculate the new values of A and B on its own. this
// can't be parallelized

//-have an asm gcd. nothing else is asm. will use gmp for everything else
//-the asm gcd takes unsigned inputs where a>=b. it returns unsigned outputs.
//its inputs are zero-padded to a fixed size -it modifes its inputs and returns
//a sequence of cofactor matricies -gmp has some utility functions to make this
//work easily. gmp can also calculate the new size. the resulting sign is always
//+
//--the sequence is outputted to a fixed size array of cache lines. there is
//also an output counter which should initially be 0
//-- and can be any pointer. the msb of the output counter is used to indicate
//the last output -the slave core is still used -gmp is close to optimal for the
//pentium machine so will just use it. for the fast machine, can use avx-512 if
//i have time. the gmp
//- division is still used but only to find the approximate inverse. the result
//quotient should be >= the actual quotient for exact
...
bool square_fast(form &f, const integer &d, const integer &L,
  ...
  // sometimes the nudupl code won't reduce the output all the way. if it has
  // too many bits it will get reduced by calling
  //  square_original
  if (f.a.num_bits() < max_bits_ab && f.b.num_bits() < max_bits_ab &&
      f.c.num_bits() < max_bits_c) {
...
int main(int argc, char *argv[]) {
  ...
  // array<fixed_integer<uint64, 17>, 2> ab={fixed_integer<uint64,
  // 17>(ab_start_0), fixed_integer<uint64, 17>(ab_start_1)};
  ...
  // todo //set up thread affinity. make sure they are not hyperthreads on the
...
too many bits it will get reduced by calling
    ...
    //if a<=L then this will return false; usually a has twice as many limbs as
L bool a_too_small; a_too_small=(_.a.num_limbs()<=_.L.num_limbs()+1); if
(a_too_small) { return false;
    }

    //only b can be negative
    //neither a or b can be 0; d=b^2-4ac is prime. if b=0, then
d=-4ac=composite. if a=0, then d=b^2; d>=0
    //no constraints on which is greater
    //the gcd result is 1 because d=b^2-4ac ; assume gcd(a,b)!=1 ; a=A*s ; b=B*s
; s=gcd(a,b)!=1 ; d = (Bs)^2-4Asc
    // d = B^2*s^2 - 4sac = s(B^2*s - 4ac) ; d is not prime. d is supposed to be
prime so this can't happen
    //the quadratic form might not be reduced all the way so it's possible for
|b|>a. need to swap the inputs then
    ...
    //start with <0,c> or <c,0> which is padded to 18 limbs so that the
multiplications by 64 bits are exact (same with sums)
    //once the new values of uv are calculated, need to reduce modulo a, which
is 17 limbs and has been normalized already
    //-the normalization also left shifted c
    //reducing modulo a only looks at the first couple of limbs so it has the
same efficiency as doing it at the end
    ...
    // b*t + c*v1 === b*v1*k + c*v1 === v1(b*k+c) === v1(-U0*c*b+c) ===
c*v1*(1-U0*b) === c*v1*(1-1) === 0 mod a
    // b*t + c*v1 = b*(u0*a + v1*k) + c*v1 = b*u0*a + v1(b*k + c) = b*u0*a +
v1(c - b*(U0*c+q*a))
    // = b*u0*a + v1(c - b*U0*c - b*q*a) = b*u0*a + v1(c - (1-V0*a)*c - b*q*a) =
b*u0*a + v1(V0*a*c - b*q*a)
    // = a*(b*u0 + v1(V0*c - b*q))
