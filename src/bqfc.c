...
int bqfc_decompr(mpz_t out_a, mpz_t out_b, const mpz_t D,
                 const struct qfb_c *c) {
  int ret = 0;
  mpz_t tmp, t, t_inv, d;
 ...
 */
int bqfc_serialize_only(uint8_t *out_str, const struct qfb_c *c,
                        size_t d_bits) {
...
int bqfc_deserialize_only(struct qfb_c *out_c, const uint8_t *str,
                          size_t d_bits) {
...
int bqfc_get_compr_size(size_t d_bits) { return (d_bits + 31) / 32 * 3 + 4; }
...
static int bqfc_verify_canon(mpz_t a, mpz_t b, const uint8_t *str,
                             size_t d_bits) {
...
int bqfc_deserialize(mpz_t out_a, mpz_t out_b, const mpz_t D,
                     const uint8_t *str, size_t size, size_t d_bits) {
  struct qfb_c f_c;
  int ret;
