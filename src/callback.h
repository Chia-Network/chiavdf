...
public:
  ...
  void reduce(form &inf) { reducer->reduce(inf); }
...
public:
  OneWesolowskiCallback(integer &D, form &f, uint64_t wanted_iter)
...
public:
  TwoWesolowskiCallback(integer &D, form f) : WesolowskiCallback(D) {
    int space_needed =
        kSwitchIters / 10 + (kMaxItersAllowed - kSwitchIters) / 100;
    forms.reset(new form[space_needed]);
    forms[0] = f;
    kl = 10;
    switch_iters = -1;
  ...
  form *GetForm(uint64_t power) { return &(forms[GetPosition(power)]); }

  bool LargeConstants() { return kl == 100; }
...
public:
  FastAlgorithmCallback(int segments, integer &D, form f,
    ...
    for (int i = 0; i < segments - 2; i++) {
      buckets_begin.push_back(buckets_begin[buckets_begin.size() - 1] +
                              bucket_size2 * window_size);
    }
    int space_needed =
        window_size * (bucket_size1 + bucket_size2 * (segments - 1));
    forms.reset(new form[space_needed]);
