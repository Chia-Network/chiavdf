...
template <int size>
void gcd_unsigned_slow(array<fixed_integer<uint64, size>, 2> &ab,
                       array<fixed_integer<uint64, size>, 2> &uv, int &parity,
                       const fixed_integer<uint64, size> &threshold =
                           fixed_integer<uint64, size>(integer(0))) {
...
// todo
// test this by making two numbers that have a specified quotient sequence. can
// add big quotients then to generate numbers with a certain quotient sequence:
...
template <int size>
void gcd_unsigned(array<fixed_integer<uint64, size>, 2> &ab,
                  array<fixed_integer<uint64, size>, 2> &uv, int &parity,
                  fixed_integer<uint64, size> threshold =
                      fixed_integer<uint64, size>(integer(0))) {
  ...
  while (true) {
    ...
    if (debug_output) {
      ...
      for (int x = 0; x < size; ++x)
        print("a limb", x, ab[0][x]);
      print("");

      for (int x = 0; x < size; ++x)
        print("b limb", x, ab[1][x]);
      print("");

      for (int x = 0; x < size; ++x)
        print("threshold limb", x, threshold[x]);
      print("");
    }
    ...
    if (ab[1] <= threshold) {
      if (debug_output)
        print("ab[1]<=threshold");
      break;
    }

    // there is a cached num limbs for a. the num limbs for b and ab_threshold
    // is smaller to calculate the new cached num limbs: -look at previous
    //value. if limb is 0, go on to the next lowest limb. a cannot be 0 but
    //should still tolerate this without crashing -unroll this two times -if
    //more than 2 iterations are required, use a loop -a can only decrease in
    //size so its true size can't be larger -this also calculates the head limb
    //of a. need the top 3 head limbs. they are 0-padded if a is less than 3
    //nonzero limbs -the 3 head limbs are used to do the shift -this also
    //truncates threshold and compares a[1] with the truncated value. it will
    //exit if they are equal. this is not
    //  exactly the same as the c++ code
    //-should probably implement this in c++ first then to make the two codes
    //the same
    ...
    array<uint128, 2> ab_head = {
        uint128(ab[0].window(shift_amount)) |
            (uint128(ab[0].window(shift_amount + 64)) << 64),
        uint128(ab[1].window(shift_amount)) |
            (uint128(ab[1].window(shift_amount + 64)) << 64)};
    // assert((ab_head[0]>>127)==0);
    // assert((ab_head[1]>>127)==0);

    uint128 threshold_head =
        uint128(threshold.window(shift_amount)) |
        (uint128(threshold.window(shift_amount + 64)) << 64);
    // assert((threshold_head>>127)==0);

    // don't actually need to do this
    // it will compare threshold_head with > so it will already exit if they are
    // equal if (shift_amount!=0) {
    //     ++threshold_head;
    // }

    if (debug_output)
      print("a_num_bits:", a_num_bits);
    if (debug_output)
      print("a last index:", (a_num_bits + 63 / 64) - 1);
    if (debug_output)
      print("shift_amount:", shift_amount);
    if (debug_output)
      print("ab_head[0]:", uint64(ab_head[0]), uint64(ab_head[0] >> 64));
    if (debug_output)
      print("ab_head[1]:", uint64(ab_head[1]), uint64(ab_head[1] >> 64));
    if (debug_output)
      print("threshold_head:", uint64(threshold_head),
            uint64(threshold_head >> 64));

    array<array<uint64, 2>, 2> uv_uint64;
    int local_parity; // 1 if odd, 0 if even
    if (gcd_128(ab_head, uv_uint64, local_parity, shift_amount != 0,
                threshold_head)) {
      // int local_parity=(uv_double[1][1]<0)? 1 : 0; //sign bit
      bool even = (local_parity == 0);

      if (debug_output)
        print("u:", uv_uint64[0][0], uv_uint64[1][0]);
      if (debug_output)
        print("v:", uv_uint64[0][1], uv_uint64[1][1]);
      if (debug_output)
      ...
      // can use a_num_bits to make these smaller. this is at most a 2x speedup
      // for these mutliplications which probably doesn't matter can do this
      // with an unsigned subtraction and just swap the pointers
      //
      // this is an unsigned subtraction with the input pointers swapped to make
      // the result nonnegative
      //
      // this uses mulx/adox/adcx if available for the multiplication
      // will unroll the multiplication loop but early-exit based on the number
      // of limbs in a (calculated before). this gives each branch its own
      // branch predictor entry. each branch is at a multiple of 4 limbs. don't
      // need to pad a
      int_t a_new_1 = ab[0];
      a_new_1 *= uv_00;
      a_new_1.set_negative(!even);
      int_t a_new_2 = ab[1];
      a_new_2 *= uv_01;
      a_new_2.set_negative(even);
      int_t b_new_1 = ab[0];
      b_new_1 *= uv_10;
      b_new_1.set_negative(even);
      int_t b_new_2 = ab[1];
      b_new_2 *= uv_11;
      b_new_2.set_negative(!even);

      // both of these are subtractions; the signs determine the direction. the
      ...
      // the array is processed while it is being generated so it is cache line
      // aligned, has a counter, etc
  ...
  if (test_asm_run) {
    ...
    int error_code = hasAVX2() ? asm_code::asm_avx2_func_gcd_unsigned(&asm_data)
    ...
    if (error_code == 0) {
      ...
      assert(asm_data.iter >= 0 &&
             asm_data.iter <=
                 asm_max_iter); // total number of iterations performed
      bool is_even = ((asm_data.iter - 1) & 1) ==
                     0; // parity of last iteration (can be -1)
      ...
      assert(asm_uv_counter ==
             asm_uv_counter_start + asm_data.iter -
                 1); // the last iteration that updated the counter is iter-1
    } else {
      if (!valid) {
