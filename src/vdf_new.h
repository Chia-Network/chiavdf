...
void generator_for_discriminant(const integer &d, integer &a, integer &b,
                                integer &c) {
  a = 2;
  b = 1;
...
//  |b| <= a
//  |a|< = sqrt(-d/3) <= sqrt(|d|)
//  |c| = |(b^2-d)/(4a)| <= |b^2-d| = |b^2+|d|| <= |a^2+|d|| <= |-d/3+|d|| =
//  ||d|/3+|d|| = |2/3*|d|| <= |d| a and b have half as many bits as d (rounded
//  up). c can have as many bits as d (but it is usually half) |ac| =
//  |(b^2-d)/(4)| <= |b^2-d| <= |d| |bc| <= |ac| <= |d| b is odd:
//   assume b=2n (even)
...
struct form {
  ...
  static form from_abd(const integer &t_a, const integer &t_b,
                       const integer &d) {
    form res;
    res.a = t_a;
    res.b = t_b;
  ...
  bool check_valid(const integer &d) { return b * b - integer(4) * a * c == d; }

  void assert_valid(const integer &d) { assert(check_valid(d)); }
  ...
  int hash() const {
    ...
    return int((res >> 4) &
               ((1ull << 31) - 1)); // ignoring some of the lower bits because
                                    // they might not be random enough
  }
};
...
void solve_linear_congruence(const integer &a, const integer &b,
                             const integer &m, integer &mu, integer &v,
                             int index = 0) {
  // g = gcd(a, m), and da + em = g
  // one round of the euclidean algorithm will equalize the sizes of the inputs;
  // a is signed and m is unsigned
  ...
  mu = (q * d) % m; // N+M bits mod M bits => M bits ; signed*signed mod
                    // unsigned = unsigned
  v = m / g; // M bits unsigned
}

// f1.a,f1.b,f2.a,f2.b are N bits and f1.c,f2.c are 2N bits. a/c are unsigned
// and b is signed
form multiply(const form &f1, const form &f2) {
  form f3;

  integer g =
      (f2.b + f1.b) / integer(2); // N bits signed; sum is odd+odd which is even
  integer h =
      (f2.b - f1.b) / integer(2); // N bits signed; sum is odd-odd which is even
  ...
  solve_linear_congruence(
      t * u,            // 2N bits signed
      h * u + s * f1.c, // f1.a * f1.c is 2N bits; 2N+1 bits; signed+unsigned =
                        // signed
  ...
  solve_linear_congruence(t * constant_factor, // 3N bits signed
  ...
  integer m =
      (t * u * k - h * u - s * f1.c) /
      (s *
       t); // 6N bits divided by 2N bits => 6N bits ; signed / unsigned = signed
