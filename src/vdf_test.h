bool square_fast_impl(form &f, const integer &D, const integer &L,
  ...
  fixed_integer<uint64, 17> v2_int;
  {
    ...
    // d=-4ac=composite. if a=0, then d=b^2; d>=0 no constraints on which is
    // greater
  ...
  // multiplications by 64 bits are exact (same with sums) once the new values
  // of uv are calculated, need to reduce modulo a, which is 17 limbs and has
  // been normalized already -the normalization also left shifted c reducing
  // modulo a only looks at the first couple of limbs so it has the same
  // efficiency as doing it at the end -it does require computing the inverse of
  //a a bunch of times which is slow. this will probably slow it down by 2x-4x
  //--can avoid this by only reducing every couple of iterations
  integer k = (-v2 * c) % a;
  fixed_integer<uint64, 17> k_int =
      fixed_integer<uint64, 33>(-v2_int * c_int) % a_int;
  ...
  const bool same_cofactors =
      false; // gcd and xgcd_parital can return slightly different results
  ...
  // todo
  // can speed the following operations up with simd (including calculating C
  // but it is done on the slave core) division by a can be replaced by
  // multiplication by a inverse. this takes the top N bits of the numerator and
  // denominator inverse
  //  where N is the number of bits in the result
  // if this is done correctly, the calculated result withh be >= the actual
  // result, and it will be == almost all of the time to detect if it is >, can
  // calculate the remainder and see if it is too high. this can be done by the
  // slave core during the
  //  next iteration
  // most of the stuff is in registers for avx-512
  // the slave core will precalculate a inverse. it is already dividing by a to
  // calculate c this would get rid of the 8x8 batched multiply but not the
  // single limb multiply, since that is still needed for gcd for the cofactors
  // which are calculated on the slave core, can use a tree matrix
  // multiplication with the avx-512 code for the pentium processor, the adox
  // instruction is banned so the single limb multiply needs to be changed the
  // slave core can calculate the inverse of co1 while the master core is
  // calculating A for the modulo, the quotient has about 15 bits. can probably
  // calculate the inverse on the master core then since the division
  ...
  // TRACK_MAX(co1*((b*k_copy-c*co1)/a)); // co1*((b*k_copy-c*co1)/a), 2.00585,
  // 1
  ...
  // TRACK_MAX((a*k_copy-A*co2)*integer(2)); //
  // (a*k_copy-A*co2)*integer(2), 3.00585, 1
  // TRACK_MAX(((a*k_copy-A*co2)*integer(2))/co1); //
  // ((a*k_copy-A*co2)*integer(2))/co1, 2.03515, 1
  // TRACK_MAX(((a*k_copy-A*co2)*integer(2))/co1 - b); //
  // ((a*k_copy-A*co2)*integer(2))/co1 - b, 2.03515, 1

  integer B = (((a * k_copy - A * co2) * integer(2)) / co1 - b) %
              (A * integer(2)); //[exact]
  // TRACK_MAX(B); // B, 2.00585, 0
  ...
  integer C = ((B * B - D) / A) >>
              2; //[division is exact; right shift is truncation towards 0; can
                 //be negative. right shift is exact]
