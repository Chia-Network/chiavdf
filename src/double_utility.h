...
struct double_bits {
  ...
  void set_exponent(int v) { exponent = v + 1023; }
...
// the cpu has to handle values of i that are above 2^52-1 so the built in
// instruction is slower than doing it this way can make this add a shift easily
double double_from_int(uint64 i) {
  ...
  // res_b.exponent+=52; //can't overflow; 1 uint64 add without shifts. can also
  // use a 32/16 bit add or a double multiply double res=res_b.to_double();
