...
// todo //test hit rate
void divide_table(reg_alloc regs, reg_scalar a, reg_scalar b, reg_scalar q,
                  reg_scalar r) {
  EXPAND_MACROS_SCOPE;
  ...
  APPEND_M(str("SUB `q, #",
               to_hex(divide_table_index_bits -
                      1))); // b_shift = bsr(b)-(divide_table_index_bits-1)
...
#ifdef CHIAOSX
  APPEND_M(str("LEA RCX, [RIP+divide_table]")); // b_approx_inverse =
                                                // divide_table[b_approx]
  APPEND_M(str("MOV RAX, [RCX+RAX*8]"));
#else
  APPEND_M(str("MOV RAX, [divide_table+RAX*8]")); // b_approx_inverse =
                                                  // divide_table[b_approx]
#endif
  ...
  if (!asm_output_common_case_only) {
    ...
    APPEND_M(str("MOV RDX, `a"));
    APPEND_M(str("SAR RDX, #",
                 to_hex(63))); // all 1s if negative, all 0s if nonnegative
...
// a is int64
void gcd_64_iteration(reg_alloc regs, reg_vector c_gcd_mask,
                      array<reg_scalar, 2> a, array<reg_vector, 2> uv,
                      reg_scalar ab_threshold, string early_exit_label) {
  EXPAND_MACROS_SCOPE;
  ...
  APPEND_M(
      str("VPSUBQ `new_uv_1, `uv_0, `new_uv_1")); // new_uv_1 = uv[0] - q*uv[1]

  // overflow checking:
  //-the carry_mask bits must be all 0s or all 1s for each 64-bit entry
  //-if 1<<data_size is added, the carry_mask bits must be all 0s (negative) or
  //1<<data_size with the rest 0 (nonnegative) -can add 1<<data_size, then check
  //the carry_mask except the last bit
  APPEND_M(str("VPADDQ `tmp_1, `new_uv_1, #",
               constant_address_uint64(1ull << data_size, 1ull << data_size)));
  APPEND_M(str("VPTEST `tmp_1, #",
               constant_address_uint64(carry_mask & (~(1ull << data_size)),
                                       carry_mask & (~(1ull << data_size)))));
...

  {
    ...
    APPEND_M(str(
        "VPSUBQ `tmp_1, `tmp_1, `tmp_2")); // tmp_1 =
                                           // a[0]+new_uv[0]-(a[1]+new_uv[1])

    APPEND_M(
        str("VPOR `tmp_1, `tmp_1, `tmp_2")); // sign is 1 if tmp_1<0 or tmp_2<0

    // approximate: ZF set if both signs of tmp_1 are 0 (i.e tmp_1>=0 and
    // tmp_2>=0 for both lanes) exact: ZF set always
