...
struct asm_integer {
  // if a sign limb exists, it is one qword before this address. the data limbs
  // are after this address
  reg_scalar addr_base;

  // the asm_integer functions only use addr_base. this is used to assign
  // addr_base if it needs to be allocated
  ...
  // end_index should initially be >= the number nonzero of limbs minus 1, but
  // not more than size-1 if the integer is 0, end_index should initially be at
  // least 0 and the returned end_index is 0 regs: 3x scalar
  void update_end_index(reg_alloc regs, reg_scalar end_index) {
    EXPAND_MACROS_SCOPE;

    assert(size % 4 == 0);
    assert(addr_offset ==
           0); // can temporarily modify addr_base if this is false
  ...
  // regs: 1x scalar
  void extract_head_at(reg_alloc regs, reg_scalar head_start,
                       array<reg_scalar, 3> res) {
    EXPAND_MACROS_SCOPE;
    ...
    APPEND_M(str("LEA `tmp_addr, [`addr_base+`head_start*8+#]",
                 to_hex(addr_offset)));
  ...
  void mul_add_bmi(reg_alloc regs, asm_integer a, reg_scalar b, asm_integer c,
                   bool invert_output, bool carry_in_is_1) {
    EXPAND_MACROS_SCOPE;
  ...
  void mul_add_slow(reg_alloc regs, asm_integer a, reg_scalar b, asm_integer c,
                    bool invert_output, bool carry_in_is_1) {
    EXPAND_MACROS_SCOPE;
  ...
  void mul_add(const reg_alloc &regs, asm_integer a, reg_scalar b,
               asm_integer c, bool invert_output, bool carry_in_is_1) {
    EXPAND_MACROS_SCOPE;
...
// sets res to the right shift amount required for the uppermost limb to be 0.
// this is between 0 and 64 inclusive regs: 1x scalar
void calculate_shift_amount(reg_alloc regs, array<reg_scalar, 3> limbs,
                            reg_scalar res) {
  EXPAND_MACROS_SCOPE;
...
// regs: 1x scalar
// in-place
void shift_right(reg_alloc regs, array<reg_scalar, 3> limbs,
                 reg_scalar amount) {
  EXPAND_MACROS_SCOPE;
...
//
// all of the integers should have spilled addresses with offsets of 0. all of
// their sizes should be the same the input a and b values should go into
// spill_a and spill_b. spill_a_2 and spill_b_2 should be uninitialized
// spill_iter will be between -1 and max_iterations
// the final a value is in spill_a if spill_iter is odd, otherwise is is in a_2.
// same with b
//
...
//-if iter is -1, only exit_flag is initialized and the rest have undefined
//values -if exit_flag is 1, this is the final result
//
// no more than max_iterations+1 results will be outputted. there will be an
// error if there are more results than this (this includes iteration -1)
//
// spill_a_end_index must be < a's size and >= 0. any limbs past this must be 0
// for a, b, and threshold, but only up to the next
//  multiple of 4 limbs. (e.g. if spill_a_end_index is 6, there are 7 limbs so
//  the 8th limb must be 0 and the rest can be uninitialized)
//
// the return value of iter is the total number of iterations performed, which
// is at least 0. iter-1 is the parity of the last iteration
void gcd_unsigned(reg_alloc regs_parent, asm_integer spill_a,
                  asm_integer spill_b, asm_integer spill_a_2,
                  asm_integer spill_b_2, asm_integer spill_threshold,
                  reg_spill spill_uv_counter_start,
                  reg_spill spill_out_uv_counter_addr,
                  reg_spill spill_out_uv_addr, reg_spill spill_iter,
                  reg_spill spill_a_end_index, int max_iterations) {
  EXPAND_MACROS_SCOPE_PUBLIC;

  track_asm("gcd_unsigned");

  int int_size = spill_a.size;
  assert(spill_a.addr_offset == 0 && spill_b.addr_offset == 0 &&
         spill_threshold.addr_offset == 0);
  assert(spill_a.addr_base.value == -1 && spill_b.addr_base.value == -1 &&
         spill_threshold.addr_base.value == -1);
  assert(spill_a_2.addr_offset == 0 && spill_b_2.addr_offset == 0);
  assert(spill_a_2.addr_base.value == -1 && spill_b_2.addr_base.value == -1);
  assert(spill_a.size == int_size && spill_b.size == int_size &&
         spill_threshold.size == int_size);
  ...
  reg_spill spill_threshold_128 =
      regs_parent.bind_spill(m, "spill_threshold_128", 16, 8);
  ...
  gcd_128(
      regs_parent, {spill_a_128, spill_b_128}, {spill_u_0, spill_u_1},
      {spill_v_0, spill_v_1}, spill_parity, spill_is_lehmer,
      spill_threshold_128,
...

  {
    ...
    for (int end_index = 0; end_index < int_size; ++end_index) {
      ...
      APPEND_M(str(".quad ") + asmprefix +
               str("multiply_uv_size_#", mapped_size));
    }
    APPEND_M(str(".text"));
  ...
  reg_scalar shift_right_amount =
      regs_parent.bind_scalar(m, "shift_right_amount");
  ...
  APPEND_M(str("#:", loop_start));
  {
    ...
    calculate_shift_amount(regs, {a_head_0, a_head_1, a_head_2},
                           shift_right_amount);
  ...
  // clobbers is_lehmer
  {
    ...
    reg_scalar threshold_head_0 =
        regs.bind_scalar(m, "threshold_head_0", reg_rdx);
    ...
    threshold.extract_head_at(
        regs, a_head_start,
        {threshold_head_0, threshold_head_1, threshold_head_2});
    shift_right(regs, {threshold_head_0, threshold_head_1, threshold_head_2},
                shift_right_amount);
    ...
    APPEND_M(
        str("JNC #", track_asm("gcd_unsigned error: a_head<=threshold_head",
                               m.alloc_error_label())));
    ...
    APPEND_M(
        str("OR `threshold_head_0, `is_lehmer")); // ZF = (threshold_head'==0 &&
                                                  // is_lehmer)
    APPEND_M(str(
        "JZ #",
        track_asm("gcd_unsigned error: b_head==threshold_head and is_lehmer",
                  m.alloc_error_label())));
  }
...

  {
    ...
    // done assigning the data; can now increment the counter. this is not
    // atomic because only this thread can write to the counter (the counter
    //must be 8-aligned) x86 uses acq_rel ordering on all of the loads and
    // stores so no fences are required
    ...
    APPEND_M(
        str("JGE #", track_asm("gcd_unsigned error: max_iterations exceeded",
                               m.alloc_error_label())));
  }
