...
// regs: RCX, 1x scalar
void shift_right(const reg_alloc &regs, array<reg_scalar, 2> v,
                 reg_scalar amount, reg_scalar res, reg_scalar tmp_rcx,
                 reg_scalar tmp_res_2) {
  EXPAND_MACROS_SCOPE;
...
void dot_product_exact(reg_alloc regs, array<reg_scalar, 2> a,
                       array<reg_scalar, 2> b, reg_scalar out,
                       string overflow_label) {
  EXPAND_MACROS_SCOPE;
  m.bind(a, "a");
  m.bind(b, "b");
  m.bind(out, "out");
...
// ab and ab_threshold reg_spill are 16 bytes (lsb first), 8 byte aligned. all
// others are 8 bytes parity is 1 if odd, else 0 is_lehmer is 1 if true, else 0
// u, v, and parity are outputs
// regs: 15x scalar, 16x vector (i.e. all of the registers except RSP)
void gcd_128(reg_alloc regs_parent, array<reg_spill, 2> spill_ab_start,
             array<reg_spill, 2> spill_u, array<reg_spill, 2> spill_v,
             reg_spill spill_parity, reg_spill spill_is_lehmer,
             reg_spill spill_ab_threshold, string no_progress_label) {
  ...
  reg_vector vector_ab_threshold =
      regs_parent.bind_vector(m, "vector_ab_threshold");
  ...
  if (use_divide_table) {
    ...
    gcd_64_iteration(regs_parent, vector_is_lehmer, {new_u_0, new_u_1},
                     {vector_u, vector_v}, new_v_0, base_exit_label);
    ...
    gcd_base_continued_fraction(
        regs_parent, vector_ab, vector_u, vector_v, vector_is_lehmer,
        vector_ab_threshold, track_asm("gcd_128 base no progress", exit_label));
  }

  {
    ...
    auto abs_tmp_a = [&]() {
      if (use_divide_table) {
        ...
        APPEND_M(str("PSRAD `tmp_b, 32")); // high 32 bits = sign bit ; low 32
                                           // bits = undefined
        APPEND_M(str("PSHUFD `tmp_b, `tmp_b, #",
                     to_hex(0b11110101))); // move high 32 bits to low 32 bits
    ...
    dot_product_exact(regs, {m_0_0, m_0_1}, {tmp_0, tmp_1}, new_u_0,
                      track_asm("gcd_128 uv overflow", exit_label));
    dot_product_exact(regs, {m_1_0, m_1_1}, {tmp_0, tmp_1}, new_u_1,
                      track_asm("gcd_128 uv overflow", exit_label));

    APPEND_M(str("MOV `tmp_0, `spill_v_0"));
    APPEND_M(str("MOV `tmp_1, `spill_v_1"));
    dot_product_exact(regs, {m_0_0, m_0_1}, {tmp_0, tmp_1}, new_v_0,
                      track_asm("gcd_128 uv overflow", exit_label));
    dot_product_exact(regs, {m_1_0, m_1_1}, {tmp_0, tmp_1}, new_v_1,
                      track_asm("gcd_128 uv overflow", exit_label));
  }
...

  {
    ...
    auto dot_product_subtract = [&](string a0, string a1, string b0, string b1,
                                    string u, string v, string out0,
                                    string out1) {
      //[RDX:RAX]=a0*u
    ...
    dot_product_subtract("ab_start_0_0", "ab_start_0_1", "ab_start_1_0",
                         "ab_start_1_1", "new_u_0", "new_v_0", "new_ab_0_0",
                         "new_ab_0_1");
    ...
    dot_product_subtract("ab_start_1_0", "ab_start_1_1", "ab_start_0_0",
                         "ab_start_0_1", "new_v_1", "new_u_1", "new_ab_1_0",
                         "new_ab_1_1");
  ...
  // SBB a1,b1 ; sets CF if b>a. sets ZF if b==a. may set ZF if b<a (e.g. a1==0;
  // b1==0; b0<a0) CF set: a<b CF cleared: a>=b need to swap the order for <=
  // and >

  {
    ...
    // -uv_10 +uv_11
    //
    // uint128 v_delta=uint128(v_1)+uint128(v_0); //even: positive. odd:
    // negative uint128 u_delta=uint128(u_1)+uint128(u_0); //even: negative.
    // odd: positive
    //
    ...
    APPEND_M(str("CMOVE `ab_threshold_0, `is_lehmer")); // if moved,
                                                        // is_lehmer==0
    ...
    //[uv_delta_1:uv_delta_0]=uv_delta_0 + uv_delta_1 //v_delta if even, else
    //u_delta
    ...
    APPEND_M(str("JC #", track_asm("gcd_128 lehmer fail ab_delta<uv_delta",
                                   exit_label)));
    ...
    APPEND_M(str("JC #", track_asm("gcd_128 lehmer fail new_ab[1]<b_new_min",
                                   exit_label)));
    ...
    APPEND_M(
        str("JNC #", track_asm("gcd_128 went too far ab_threshold>=new_ab[0]",
                               exit_label)));
...

  {
    ...
    // int shift_amount=a_num_bits-gcd_base_bits; [shift amount can't exceed
    // 128-gcd_base_bits] if (shift_amount<0) {
    //     shift_amount=0;
    // }
    ...
    if (!use_divide_table) {
#ifdef CHIAOSX
      APPEND_M(str("LEA `tmp_3, [RIP+#]",
                   constant_address_uint64(0ull, 0ull, false)));
      APPEND_M(str("LEA `tmp_0, [RIP+#]",
                   constant_address_uint64(~(0ull), ~(0ull), false)));
#else
      APPEND_M(str("MOV `tmp_3, OFFSET FLAT:#",
                   constant_address_uint64(0ull, 0ull, false)));
      APPEND_M(str("MOV `tmp_0, OFFSET FLAT:#",
                   constant_address_uint64(~(0ull), ~(0ull), false)));
#endif
    } else {
#ifdef CHIAOSX
      APPEND_M(str("LEA `tmp_3, [RIP+#]",
                   constant_address_uint64(gcd_mask_exact[0], gcd_mask_exact[1],
                                           false)));
      APPEND_M(str("LEA `tmp_0, [RIP+#]",
                   constant_address_uint64(gcd_mask_approximate[0],
                                           gcd_mask_approximate[1], false)));
#else
      APPEND_M(str("MOV `tmp_3, OFFSET FLAT:#",
                   constant_address_uint64(gcd_mask_exact[0], gcd_mask_exact[1],
                                           false)));
    ...
    // if (shift_amount!=0) {
    //     ++ab_threshold_double; [can do this with integers because the shifted
    //     ab_threshold has to fit in a double exactly] a is larger than
    //     ab_threshold
    // }
    ...
    shift_right(regs, {ab_threshold_0, ab_threshold_1}, tmp_1, new_v_0, tmp_3,
                tmp_2);
  ...
  APPEND_M(
      str("JMP #", track_asm("gcd_128 premature exit", exit_iter_0_label)));
