...
void ApproximateParameters(uint64_t T, int &l, int &k) {
  ...
  k = std::max(std::round(log(intermediate) - log(log(intermediate)) + 0.25),
               1.0);
}
...
form GenerateWesolowski(form &y, form &x_init, integer &D,
                        PulmarkReducer &reducer,
                        std::vector<form> &intermediates,
                        uint64_t num_iterations, uint64_t k, uint64_t l) {
...
std::vector<uint8_t> ProveSlow(integer &D, form &x, uint64_t num_iterations) {
  ...
  form proof =
      GenerateWesolowski(y, x, D, reducer, intermediates, num_iterations, k, l);
