...
// GMP integers:
//-the number of limbs is passed with a register. the sign is the sign of the
//integer. "0" has zero limbs -the limbs are stored little endian in an array
//which is 64-aligned and has enough memory for the max allowed number of limbs
//-any unused values in the array are 0

// AVX-512 integers:
//-the number of limbs is fixed, but doesn't have to be a multiple of the AVX
//vector size -the sign is stored in a register (0 is positive, -1 is negative)
//-the data is stored in memory in little endian order. each limb has 52 bits
//and any unused bits are 0
//--any unused limbs are 0 and there are 64 zero padding bytes before the start
...
struct gmp_integer {
  ...
  int max_num_limbs =
      -1; // will write past the end of this if it isn't a multiple of 8
...
void apply_shifts(reg_alloc regs, vector<reg_vector> inputs, reg_vector output,
  ...
  assert(commands[0].source_indexes.size() ==
         commands[0].is_right_shift.size());

  bool is_first = true;

  for (int shift_index = 0; shift_index < commands[0].source_indexes.size();
       ++shift_index) {
    // each input vector is permutated and the results are blended together
    // the final result has the correct values at each slot, except that unused
    // values are uninitialized the result is then shifted and accumulated into
    // the output with a bitwise or any unused values are shifted by 64 to zero
    // them out
    ...
    // any unused values are already processed and will be uninitialized after
    // blending
    ...
    for (int x = 0; x < 8; ++x) {
      ...
      processed[x] =
          (c_source_index < 0 || c_source_index >= inputs.size() * 8 ||
           c_shift_amount >= 64);

      // any unused inputs will be uninitialized after blending. this will make
      // them have a value of 0
    ...
    // the indexes need to be grouped by input vector and an offset needs to be
    // calculated for each permutate operation
    while (true) {
      int input_index = -1;
      for (int x = 0; x < 8; ++x) {
        int c_source_index = commands[x].source_indexes.at(shift_index);
    ...
    // the VPERMI2Q instruction requires at least two inputs, so a second
    // redundant input is added if necessary
    ...
    // the first two inputs are blended together into an intermediate buffer
    {
      ...
      APPEND_M(str("VMOVDQU64 `buffer_0, #",
                   constant_address_avx512_uint64(perm_indexes)));
      APPEND_M(str("VPERMI2Q `buffer_0, `inputs_#, `inputs_#",
                   source_inputs.at(0), source_inputs.at(1)));
    }
    ...
    for (int source_offset_index = 2;
         source_offset_index < source_offsets.size(); ++source_offset_index) {
      array<uint64, 8> perm_indexes;
      for (int x = 0; x < 8; ++x) {
      ...
      APPEND_M(str("VMOVDQU64 `buffer_#, #", next_buffer_index,
                   constant_address_avx512_uint64(perm_indexes)));
      APPEND_M(str("VPERMI2Q `buffer_#, `buffer_#, `inputs_#",
                   next_buffer_index, current_buffer_index,
                   source_inputs.at(source_offset_index)));
    }

    // finally calculate the shifts and do a bitwise or with the output (on
    // subsequent iterations)
    APPEND_M(str("# #, `buffer_#, #", (is_right_shift) ? "VPSRLVQ" : "VPSLLVQ",
                 (is_first) ? "`output" : "`buffer_0", current_buffer_index,
...
void to_avx512_integer(reg_alloc regs, gmp_integer in, avx512_integer out) {
  EXPAND_MACROS_SCOPE;
  ...
  reg_vector in_num_limbs_vector =
      regs.bind_vector(m, "in_num_limbs_vector", 512);
  ...
  for (int input_index = 0; input_index < in_num_vectors; ++input_index) {
    ...
    APPEND_M(str("VPCMPUQ k1, `in_num_limbs_vector, #, 6",
                 constant_address_avx512_uint64(input_limb_index))); // 6: >

    // load the limb values and zero out any invalid limbs
    APPEND_M(str("VMOVDQU64 `input_registers_# {k1}{z}, [`in_data+#]",
  ...
  APPEND_M(str("VMOVDQU64 `mask_vector, #",
               constant_address_avx512_uint64((1ull << 52) - 1)));

  for (int output_index = 0; output_index < ceil_div(out.num_limbs, 8);
       ++output_index) {
    array<shift_command, 8> commands;
    ...
    for (int output_offset = 0; output_offset < 8; ++output_offset) {
      ...
      commands[output_offset].shift_amounts.push_back(start_bit -
                                                      start_limb * 64);
      commands[output_offset].is_right_shift.push_back(true);

      // the bits in the next limb are used to assign the high bits that were
      // lost
      commands[output_offset].source_indexes.push_back(start_limb + 1);
      commands[output_offset].shift_amounts.push_back(
          64 - (start_bit - start_limb * 64));
...
void to_gmp_integer(reg_alloc regs, avx512_integer in, gmp_integer out) {
  EXPAND_MACROS_SCOPE;
  ...
  for (int input_index = 0; input_index < in_num_vectors; ++input_index) {
    APPEND_M(str("VMOVDQU64 `input_registers_#, [`in_data+#]", input_index,
                 input_index * 64));
  }

  APPEND_M(str("VPXORQ `zero_vector, `zero_vector, `zero_vector"));
  APPEND_M(
      str("VPXORQ `num_limbs_vector, `num_limbs_vector, `num_limbs_vector"));

  for (int output_index = 0; output_index < ceil_div(out.max_num_limbs, 8);
       ++output_index) {
    array<shift_command, 8> commands;
    ...
    for (int output_offset = 0; output_offset < 8; ++output_offset) {
      ...
      commands[output_offset].source_indexes.push_back(start_limb + 1);
      commands[output_offset].shift_amounts.push_back(
          52 - (start_bit - start_limb * 52));
      commands[output_offset].is_right_shift.push_back(false);

      // there might still be missing high bits so an additional limb might be
      // required
      commands[output_offset].source_indexes.push_back(start_limb + 2);
      commands[output_offset].shift_amounts.push_back(
          52 * 2 - (start_bit - start_limb * 52));
      commands[output_offset].is_right_shift.push_back(false);
    }
    ...
    APPEND_M(str("VMOVDQU64 `temp_vector {k1}{z}, #",
                 constant_address_avx512_uint64(output_num_limbs)));
  ...
  auto reduction_step = [&](array<uint64, 8> indexes) {
    APPEND_M(str("VMOVDQU64 `temp_vector, #",
                 constant_address_avx512_uint64(indexes)));
  ...
  APPEND_M(
      str("TEST `in_sign, `in_sign")); // ZF set if in_sign is 0. need to negate
                                       // out_num_limbs if ZF is cleared
  APPEND_M(str("CMOVNZ `out_num_limbs, `temp_scalar"));
}

// each limb can have signed 53 bit values added to or subtracted from it (i.e
// one sign bit and 52 data bits) there are 11 spare data bits so this can be
// done about 2048 times without overflow this also stores the result
void apply_carry(reg_alloc regs, avx512_integer in,
                 vector<reg_vector> in_registers) {
  EXPAND_MACROS_SCOPE;
  ...
  APPEND_M(str("VMOVDQU64 `carry_mask, #",
               constant_address_avx512_uint64(~((1ull << 52) - 1))));
  APPEND_M(str("VMOVDQU64 `permutate_indexes, #",
               constant_address_avx512_uint64({7, 8, 9, 10, 11, 12, 13, 14})));
  APPEND_M(str("VPXORQ `zero_vector, `zero_vector, `zero_vector"));

  string label_name = m.alloc_label();
  APPEND_M(str("#:", label_name));

  // need to go msb first since carries are cleared at the same time the output
  // is written
  for (int i = in_registers.size() - 1; i >= 0; --i) {
    // for each limb, find the next lowest limb (0 for the first limb)
    string low_register_name =
        (i == 0) ? "`zero_vector" : str("`in_registers_#", i - 1);
    APPEND_M(str("VMOVDQU64 `temp_vector, `permutate_indexes"));
    APPEND_M(
        str("VPERMI2Q `temp_vector, #, `in_registers_#", low_register_name, i));
    ...
    APPEND_M(
        str("VPANDNQ `in_registers_#, `carry_mask, `in_registers_#", i, i));

    // add the incoming carry to each limb. the limb has already been consumed
    // as an input
    APPEND_M(
        str("VPADDQ `in_registers_#, `in_registers_#, `temp_vector", i, i));
  ...
  APPEND_M(
      str("VMOVDQU64 [`in_data+#], `zero_vector", in_registers.size() * 64));

  // loop if any carry bit is nonzero
  APPEND_M(
      str("VPTESTMQ k1, `accumulator, `carry_mask")); // k1 true if any of the
                                                      // carry bits are nonzero
...
void add(reg_alloc regs, avx512_integer in_a, avx512_integer in_b,
         avx512_integer out, vector<reg_vector> out_registers) {
  EXPAND_MACROS_SCOPE;
  ...
  for (int i = 0; i < num_a_vectors; ++i) {
    a_registers.push_back((i < out_registers.size()) ? out_registers.at(i)
                                                     : regs.get_vector(512));
  }
  ...
  {
    // this will contain the highest index where the values of a and b are
    // different (0 if they are the same everywhere)
    APPEND_M(str("VPXORQ `pivot_index_vector, `pivot_index_vector, "
                 "`pivot_index_vector"));
    ...
    for (int i = 0; i < ceil_div(max(in_a.num_limbs, in_b.num_limbs), 8); ++i) {
      ...
      if (i < in_a.num_limbs) {
        APPEND_M(str("VMOVDQU64 #, [`in_a_data+#]", a_reg,
                     in_a.get_memory_offset(i * 8)));
      } else {
        if (!assigned_zero) {
      ...
      if (i < in_b.num_limbs) {
        APPEND_M(str("VMOVDQU64 #, [`in_b_data+#]", b_reg,
                     in_b.get_memory_offset(i * 8)));
      } else {
        if (!assigned_zero) {
      ...
      // replace each limb where "a" and "b" are different with its index. if
      // "a" and "b" are the same, 0 is used instead
      APPEND_M(str("VMOVDQU64 `A {k1}{z}, #",
                   constant_address_avx512_uint64(output_num_limbs)));

      // accumulate the max of the results, which is the highest index where "a"
      // and "b" are different
    ...
    //"a" and "b" can be compared by comparing a[pivot_index] and b[pivot_index]
    //only
    ...
    // the final sign is S and the final data is a^A+b^B+C where ^ is
    // bitwise-xor there is one lookup table with 8 entries the index is
    // a_sign*2+b_sign+3+(|a|>=|b|)? 4 : 0 each table entry is 4 64-bit scalars
    // (S, A, B, C). vector loads use a broadcast. each load is register+offset
    //|a|<|b|:
    //-a, -b: S=-, A= 0, B= 0, C=0
    //-a, +b: S=+, A=~0, B= 0, C=1
    //+a, -b: S=-, A=~0, B= 0, C=1
    ...
    APPEND_M(str("CMOVAE `temp_0, `temp_1")); // compares a[pivot_index] with
                                              // b[pivot_index]

    // temp_1 has the address of the table entry
    APPEND_M(str("SHL `temp_0, 5")); // multiply by 32 to convert the index to a
                                     // byte offset
#ifdef CHIAOSX
  ...
  for (int i = 0; i < num_out_vectors; ++i) {
    ...
    if (i >= num_a_vectors) {
      ...
      APPEND_M(
          str("VPXORQ `out_registers_#, `out_registers_#, `out_registers_#", i,
              i, i));
    } else if (i >= num_b_vectors) {
      // b=0, so the result is a or -a
      // the result can't be -a because the final result must be nonnegative
      APPEND_M(
          str("VPMOVQ `out_registers_#, `out_registers_#, `out_registers_#", i,
              i, i));
    } else {
      APPEND_M(
          str("VPXORQ `temp_vector, `a_registers_#, `A", i)); // temp_vector=a^A
      APPEND_M(str("VPADDQ `out_registers_#, `temp_vector, `C",
                   i)); // out=a^A+C ; this also clobbers "a"
      APPEND_M(
          str("VPXORQ `temp_vector, `b_registers_#, `B", i)); // temp_vector=b^B
      APPEND_M(str("VPADDQ `out_registers_#, `out_registers_#, `temp_vector", i,
                   i)); // out=a^A+b^B+C
    }
  }
}
...
void multiply(reg_alloc regs, avx512_integer in_a, avx512_integer in_b,
              avx512_integer out, vector<reg_vector> out_registers_low) {
  EXPAND_MACROS_SCOPE;
  ...
  for (int i = 0; i < num_out_vectors; ++i) {
    APPEND_M(str("VPXORQ `out_registers_low_#, `out_registers_low_#, "
                 "`out_registers_low_#",
                 i, i, i));
    APPEND_M(str("VPXORQ `out_registers_high_#, `out_registers_high_#, "
                 "`out_registers_high_#",
                 i, i, i));
  }
  ...
  APPEND_M(str("VMOVDQU64 `b, #",
               constant_address_avx512_uint64(
                   {7, 8, 9, 10, 11, 12, 13, 14}))); // b = permutate_indexes
  APPEND_M(str("VPXORQ `zero_vector, `zero_vector, `zero_vector"));
  ...
  for (int i = num_out_vectors - 1; i >= 0; --i) {
    // for each limb, find the next lowest limb (0 for the first limb)
    string low_register_name =
        (i == 0) ? "`zero_vector" : str("`out_registers_high_#", i - 1);
    APPEND_M(str("VMOVDQU64 `a, `b")); // b = permutate_indexes
    APPEND_M(
        str("VPERMI2Q `a, #, `out_registers_high_#", low_register_name, i));

    // add the shifted high limbs to the low limbs
    APPEND_M(
        str("VPADDQ `out_registers_low_#, `out_registers_low_#, `a", i, i));
  }
}
