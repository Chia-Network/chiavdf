...
struct asm_function {
  ...
  // this excludes the argument regs (if any). can add them after they are done
  // being used
  reg_alloc regs;
  ...
  const vector<reg_scalar> all_save_regs = {reg_rbp, reg_rbx, reg_r12,
                                            reg_r13, reg_r14, reg_r15};
  const vector<reg_scalar> all_arg_regs = {reg_rdi, reg_rsi, reg_rdx,
  ...
  asm_function(string t_name, int num_args = 0, int num_regs = 15,
               bool align_stack = true, bool return_error_code = true) {
    EXPAND_MACROS_SCOPE;
    ...
    if (align_stack) {
      ...
      // RSP''=RSP'-64 ; still 64-aligned but now there is at least 64 bytes of
      // unused stuff [RSP'']=RSP ; store old value in unused area
