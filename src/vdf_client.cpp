...
// Best case it'll be able to proof for up to 2^36 due to 64-wesolowski
// restriction.
int segments = 8;
int thread_count = 3;
...
void WriteProof(uint64_t iteration, Proof &result, tcp::socket &sock) {
  ...
  PrintInfo("Sending proof");
  {
    ...
    boost::asio::write(
        sock, boost::asio::buffer(str_result.c_str(), str_result.size()));
  }
  PrintInfo("Sent proof");
}

void CreateAndWriteProof(ProverManager &pm, uint64_t iteration,
                         std::atomic<bool> &stop_signal, tcp::socket &sock) {
...
void CreateAndWriteProofTwoWeso(integer &D, form f, uint64_t iters,
...
void InitSession(tcp::socket &sock) {
  ...
  boost::asio::read(sock, boost::asio::buffer(initial_form_s, form_size),
                    error);
...
void SessionFastAlgorithm(tcp::socket &sock) {
  ...
  try {
    ...
    PrintInfo("Initial form: " + to_string(f.a.impl) + " " +
              to_string(f.b.impl));
    std::vector<std::thread> threads;
    const bool multi_proc_machine =
        (std::thread::hardware_concurrency() >= 16) ? true : false;
    WesolowskiCallback *weso =
        new FastAlgorithmCallback(segments, D, f, multi_proc_machine);
    FastStorage *fast_storage = NULL;
    if (multi_proc_machine) {
      fast_storage = new FastStorage((FastAlgorithmCallback *)weso);
    }
    std::atomic<bool> stopped(false);
    std::thread vdf_worker(repeated_square, f, std::ref(D), std::ref(L), weso,
                           fast_storage, std::ref(stopped));
    ProverManager pm(D, (FastAlgorithmCallback *)weso, fast_storage, segments,
                     thread_count);
    pm.start();
    ...
    while (!stopped) {
      ...
      if (iters == 0) {
        ...
        threads.push_back(std::thread(CreateAndWriteProof, std::ref(pm), iters,
                                      std::ref(stopped), std::ref(sock)));
      }
    }
  } catch (std::exception &e) {
    PrintInfo("Exception in thread: " + to_string(e.what()));
...
void SessionOneWeso(tcp::socket &sock) {
  ...
  try {
    ...
    std::thread th_prover(CreateAndWriteProofOneWeso, iter, std::ref(D), f,
                          (OneWesolowskiCallback *)weso, std::ref(stopped),
                          std::ref(sock));
  ...
  try {
    ...
    while (!stopped) {
      ...
      if (iters == 0) {
        ...
        if (!unique) {
          PrintInfo("Duplicate iteration " + to_string(iters) +
                    "... Ignoring.");
          continue;
        }
        ...
        if (threads.size() < kMaxProcessesAllowed || iters < min_iter) {
          ...
          threads.push_back(std::thread(CreateAndWriteProofTwoWeso, std::ref(D),
                                        f, iters, (TwoWesolowskiCallback *)weso,
                                        std::ref(stop_vector[threads.size()]),
                                        std::ref(sock)));
...
int main(int argc, char *argv[]) try {
  ...
  tcp::resolver::query query(tcp::v6(), argv[1], argv[2],
