...
int VerifyWesoSegment(integer &D, form x, form proof, integer &B,
                      uint64_t iters, form &out_y) {
...
void VerifyWesolowskiProof(integer &D, form x, form y, form proof,
                           uint64_t iters, bool &is_valid) {
  PulmarkReducer reducer;
  integer L = root(-D, 4);
...
bool CheckProofOfTimeNWesolowski(integer D, const uint8_t *x_s,
                                 const uint8_t *proof_blob,
                                 int32_t proof_blob_len, uint64_t iterations,
                                 uint64 disc_size_bits, int32_t depth) {
  ...
  VerifyWesolowskiProof(D, x, DeserializeForm(D, proof_blob, form_size),
