...
struct square_state_type {
  ...
  // running the gcd will advance the counter value by this much on both the
  // master and slave threads it is then advanced by 1 after the gcd results are
  // consumed
  ...
  struct phase_start_type {
    ...
    int2x as[2]; // a>=0
    int2x bs[2]; // b>=0
    alignas(64) int ab_index =
        0; // index of the start a/b values. the new values will be written to
           // the other slot in this array
    alignas(8) bool b_higher_magnitude_than_a = false; // also true if b==a
    alignas(8) uint64 num_valid_iterations = 0;
    alignas(8) bool corruption_flag =
        false; // the slave thread will set this if c is invalid
  ...
  static const int counter_start_phase_1 =
      counter_start_phase_0 + gcd_num_counter + 1;
  static const int counter_start_phase_2 =
      counter_start_phase_1 + gcd_num_counter + 1;
  ...
  struct phase_0_master_d_type {
    gcd_results_int2x
        gcd_1_0; // gcd(b,a,0); a'=1; b'=0 ; U0*b + V0*a = 1 ; U1*b + V1*a = 0
  } phase_0_master_d;
  ...
  struct phase_0_slave_d_type {
    ...
    // if b<0: <U0,U1>*=-1
    //
    // to apply a matrix Z*[X0 -Y0 ; -X1 Y1] where Z=local parity (-1 or 1) ;
    // X=local u (unsigned) ; Y=local v (unsigned): -do the matrix
    //multiplication by the column vector <U0,U1> -reduce each component of the
    //result modulo a. this can be delayed until more matrix multiplications
    //have happened if necessary
    int4x U0s[2];
    int4x U1s[2];
  ...
  struct phase_1_slave_d_type {
    gcd_results_int2x gcd_s_t; // gcd(a,k,L) ; a'=s>L ; b'=t<=L ; u0*a + v0*k =
                               // s ; u1*a + v1*k = t
  ...
  bool phase_0_master() {
    ...
    {
      TRACK_CYCLES // 185
          ab_valid = (a.num_bits() <= max_bits_ab &&
                      b.num_bits() <= max_bits_ab && a.sgn() >= 0);
    }
    if (!ab_valid) {
      return false;
    }
...

    {
      TRACK_CYCLES // 345
          gcd_1_0.get_a_start() =
          (phase_start.b_higher_magnitude_than_a) ? b : a;
  ...
  bool phase_0_slave() {
...

    {
      ...
      ++validate_iter;
      bool validate_c = (validate_iter & (validate_interval - 1)) == 0 &&
                        validate_interval != -1;
      ...
      if (!validate_c) {
        ...
        if (c_remainder.sgn() != 0) {
          assert(!is_vdf_test); // should never have corruption unless there are
                                // bugs
    ...
    while (true) {
      ...
      {
        TRACK_CYCLES // 357
            if (!gcd_1_0.get_entry(counter_start_phase_0, gcd_index,
                                   &c_entry)) {
          TRACK_CYCLES_ABORT
          return false;
        }
      ...
      if (gcd_index != 0) {
        ...
        if (calculate_k_repeated_mod &&
            num_multiplications == calculate_k_repeated_mod_interval) {
  ...
  bool phase_1_master() {
    {
      TRACK_CYCLES // 3335 (this stall doesn't matter since this thread is
                   // slower than the slave thread in this phase)
    ...
    while (true) {
      ...
      {
        TRACK_CYCLES // 396
            if (!gcd_s_t.get_entry(counter_start_phase_1, gcd_index,
                                   &c_entry)) {
          TRACK_CYCLES_ABORT
          return false;
        }
  ...
  bool phase_3_master() {
    ...
    const auto &t_t_S = phase_2_slave_d.t_t_S;
    prefetch_read(t_t_S);
  ...
  bool phase_4_master() {
    ...
    const auto &e = phase_3_slave_d.e;
    prefetch_read(e);
  ...
  bool phase_4_slave() { return true; }
  ...
  static const int counter_end =
      counter_start_phase_5; // added to counter_start to get the next counter

  void init(const integer &t_D, const integer &t_L, const integer &t_a,
            const integer &t_b) {
    int2x zero;
    zero = uint64(0ull);
  ...
  int get_counter_start(int phase) {
    int res[] = {counter_start_phase_0, counter_start_phase_1,
                 counter_start_phase_2, counter_start_phase_3,
                 counter_start_phase_4};
    return res[phase];
  }

  bool call_phase(int phase, bool is_slave) {
    decltype(&square_state_type::phase_0_master) funcs_master[] = {
        &square_state_type::phase_0_master, &square_state_type::phase_1_master,
        &square_state_type::phase_2_master, &square_state_type::phase_3_master,
        &square_state_type::phase_4_master};

    decltype(&square_state_type::phase_0_slave) funcs_slave[] = {
        &square_state_type::phase_0_slave, &square_state_type::phase_1_slave,
        &square_state_type::phase_2_slave, &square_state_type::phase_3_slave,
  ...
  // num_iterations can be less than the requested number if there was an error
  // (e.g. large gcd quotient, thread spun for too long, etc) this will set
  // num_iterations to ~uint64(0) if the return value is false
  bool assign(integer &t_a, integer &t_b, integer &t_c,
  ...
  /*
  bool assignwjb(integer& t_a, integer& t_b, integer& t_c, uint64&
  num_iterations) {
...
// the gcd loops all have maximum counters after which they'll error out, and
// the thread_state loops also have a maximum spin counter
void repeated_square_fast_work(square_state_type &square_state, bool is_slave,
                               uint64 base, uint64 iterations,
...
uint64 repeated_square_fast_multithread(square_state_type &square_state,
                                        form &f, const integer &D,
                                        const integer &L, uint64 base,
                                        uint64 iterations,
                                        INUDUPLListener *nuduplListener) {
  ...
  thread slave_thread(repeated_square_fast_work, std::ref(square_state), false,
                      base, iterations, std::ref(nuduplListener));

  repeated_square_fast_work(square_state, true, base, iterations,
                            nuduplListener);

  slave_thread
      .join(); // slave thread can't get stuck; is supposed to error out instead
...
uint64 repeated_square_fast_single_thread(square_state_type &square_state,
                                          form &f, const integer &D,
                                          const integer &L, uint64 base,
                                          uint64 iterations,
                                          INUDUPLListener *nuduplListener) {
  ...
  uint64 res;
  square_state.assign(f.a, f.b, f.c, res); // sets res to ~uint64(0) and leaves
                                           // f unchanged if there is corruption
...
uint64 repeated_square_fast(square_state_type &square_state, form &f,
                            const integer &D, const integer &L, uint64 base,
                            uint64 iterations,
                            INUDUPLListener *nuduplListener) {

  if (enable_threads) {
    return repeated_square_fast_multithread(square_state, f, D, L, base,
                                            iterations, nuduplListener);
  } else {
    return repeated_square_fast_single_thread(square_state, f, D, L, base,
                                              iterations, nuduplListener);
  }
}
