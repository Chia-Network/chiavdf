...
bool gcd_128(array<uint128, 2> &ab, array<array<uint64, 2>, 2> &uv_uint64,
             int &uv_uint64_parity, bool is_lehmer, uint128 ab_threshold = 0) {
  ...
  uv_uint64 = {array<uint64, 2>{1, 0}, array<uint64, 2>{0, 1}};
  uv_uint64_parity = 0;
  ...
  while (true) {
    if (debug_output)
      print("======== 1:", iter, uint64(ab[0]), uint64(ab[0] >> 64),
            uint64(ab[1]), uint64(ab[1] >> 64), uint64(ab_threshold),
            uint64(ab_threshold >> 64));
    ...
    if (debug_output)
      print("2:", a_zeros, a_num_bits, shift_amount);

    // print( "    gcd_128", a_num_bits );

    vector2 ab_double{double(uint64(ab[0] >> shift_amount)),
    ...
    if (debug_output)
      print("3:", ab_double[0], ab_double[1], ab_threshold_double,
            is_lehmer || (shift_amount != 0));

    vector2 ab_double_2 = ab_double;

    // this doesn't need to be exact
    // all of the comparisons with threshold are >, so this shouldn't be
    // required if (shift_amount!=0) {
    //     ++ab_threshold_double;
    // }

    // void gcd_64(vector2 start_a, pair<matrix2, vector2>& res, int&
    // num_iterations, bool approximate, int max_iterations) {
    // }

    matrix2 uv_double;
    if (!gcd_base_continued_fraction(ab_double, uv_double,
                                     is_lehmer || (shift_amount != 0),
                                     ab_threshold_double)) {
    ...
    if (debug_output)
      print("4:", uv_double[0][0], uv_double[1][0], uv_double[0][1],
            uv_double[1][1], ab_double[0], ab_double[1]);

    if (0) {
      matrix2 uv_double_2;
      if (!gcd_base_continued_fraction_2(ab_double_2, uv_double_2,
                                         is_lehmer || (shift_amount != 0),
                                         ab_threshold_double)) {
    ...
    array<array<uint64, 2>, 2> uv_double_int = {
        array<uint64, 2>{uint64(abs(uv_double[0][0])),
                         uint64(abs(uv_double[0][1]))},
        array<uint64, 2>{uint64(abs(uv_double[1][0])),
                         uint64(abs(uv_double[1][1]))}};
    ...
    if (iter == 0) {
      ...
      if (!multiply_exact(uv_double_int, uv_uint64, uv_uint64_new)) {
        print("        gcd_128 slow 1"); // calculated a bunch of quotients and
                                         // threw all of them away, which is bad
        break;
      }
    }
    ...
    if (debug_output)
      print("5:", uv_uint64_new[0][0], uv_uint64_new[1][0], uv_uint64_new[0][1],
            uv_uint64_new[1][1], uv_uint64_parity_new);
    ...
    uint128 a_new_1 = ab_start[0];
    a_new_1 *= uv_00; // a_new_1.set_negative(!even);
    uint128 a_new_2 = ab_start[1];
    a_new_2 *= uv_01; // a_new_2.set_negative(even);
    uint128 b_new_1 = ab_start[1];
    b_new_1 *= uv_11; // b_new_1.set_negative(!even);
    uint128 b_new_2 = ab_start[0];
    b_new_2 *= uv_10; // b_new_2.set_negative(even);
    ...
    // if this assert hit, one of the quotients is wrong. the base case is not
    // supposed to return incorrect quotients assert(a_new_s>=b_new_s &&
    // b_new_s>=0); commenting this out because a and b can be 128 bits now
    ...
    if (debug_output)
      print("6:", uint64(a_new), uint64(a_new >> 64), uint64(b_new),
            uint64(b_new >> 64));
    ...
    if (is_lehmer) {
      ...
      uint128 u_delta =
          uint128(uv_10) + uint128(uv_00); // even: negative. odd: positive
      uint128 v_delta =
          uint128(uv_11) + uint128(uv_01); // even: positive. odd: negative
      ...
      if (debug_output)
        print("7:", passed_even, passed_odd);

      // CMOV
      if (!(even ? passed_even : passed_odd)) {
        print("        gcd_128 slow 5"); // throwing away a bunch of quotients
                                         // because the last one is bad
        break;
      }
    }
  ...
  if (test_asm_run) {
    ...
    int error_code = hasAVX2() ? asm_code::asm_avx2_func_gcd_128(&asm_data)
