...
// all doubles are arrays with 2 entries. the high entry is first followed by
// the low entry so: b, a; u1, u0; v1, v0 is_lehmer is all 1s or all 0s.
// ab_threshold is duplicated twice
extern "C" int asm_avx2_func_gcd_base(double *ab, double *u, double *v,
                                      uint64 *is_lehmer, double *ab_threshold,
                                      uint64 *no_progress);
extern "C" int asm_cel_func_gcd_base(double *ab, double *u, double *v,
                                     uint64 *is_lehmer, double *ab_threshold,
                                     uint64 *no_progress);
#ifdef COMPILE_ASM
void compile_asm_gcd_base() {
  ...
  gcd_base_continued_fraction(regs, ab, u, v, is_lehmer, ab_threshold,
                              no_progress_label);
...
void compile_asm_gcd_128() {
  ...
  reg_spill spill_data =
      regs_parent.bind_spill(m, "spill_data", sizeof(asm_func_gcd_128_data), 8);
  ...
  gcd_128(regs_parent, {spill_data, spill_data + 16},
          {spill_data + 32, spill_data + 40},
          {spill_data + 48, spill_data + 56}, spill_data + 64, spill_data + 72,
          spill_data + 80, no_progress_label);
...
void compile_asm_gcd_unsigned() {
  ...
  reg_spill spill_data_addr = regs_parent.bind_spill(m, "spill_data_addr");
  reg_spill spill_data = regs_parent.bind_spill(
      m, "spill_data", sizeof(asm_func_gcd_unsigned_data), 8);
  ...
  gcd_unsigned(regs_parent, asm_integer(spill_data, int_size),
               asm_integer(spill_data + 8, int_size),
               asm_integer(spill_data + 16, int_size),
               asm_integer(spill_data + 24, int_size),
               asm_integer(spill_data + 32, int_size), spill_data + 40,
               spill_data + 48, spill_data + 56, spill_data + 64,
               spill_data + 72, max_iterations);
...
template <int in_max_num_limbs, int out_num_limbs>
uint64 asm_avx512_func_to_avx512_integer(uint64 in_sign, const uint64 *in_data,
                                         uint64 *out_data);

#define declare_asm_avx512_func_to_avx512_integer(in_max_num_limbs,                \
                                                  out_num_limbs)                   \
  extern "C" uint64                                                                \
      asm_avx512_func_to_avx512_integer_##in_max_num_limbs##_##out_num_limbs(      \
          uint64 in_num_limbs, const uint64 *in_data, uint64 *out_data);           \
  template <>                                                                      \
  uint64 asm_avx512_func_to_avx512_integer<in_max_num_limbs, out_num_limbs>(       \
      uint64 in_num_limbs, const uint64 *in_data, uint64 *out_data) {              \
    return asm_avx512_func_to_avx512_integer_##in_max_num_limbs##_##out_num_limbs( \
        in_num_limbs, in_data, out_data);                                          \
  }
...
#ifndef COMPILE_ASM
for_each_asm_avx512_func_to_avx512_integer(
    declare_asm_avx512_func_to_avx512_integer)
#endif

#ifdef COMPILE_ASM
    void compile_asm_avx512_to_avx512_integer(int in_max_num_limbs,
                                              int out_num_limbs) {
  EXPAND_MACROS_SCOPE;

  asm_function c_func(
      str("to_avx512_integer_#_#", in_max_num_limbs, out_num_limbs), 3, 9,
      false, false);
  reg_alloc regs = c_func.regs;
...
template <int in_num_limbs, int out_max_num_limbs>
uint64 asm_avx512_func_to_gmp_integer(uint64 in_sign, const uint64 *in_data,
                                      uint64 *out_data);

#define declare_asm_avx512_func_to_gmp_integer(in_num_limbs,                    \
                                               out_max_num_limbs)               \
  extern "C" uint64                                                             \
      asm_avx512_func_to_gmp_integer_##in_num_limbs##_##out_max_num_limbs(      \
          uint64 in_sign, const uint64 *in_data, uint64 *out_data);             \
  template <>                                                                   \
  uint64 asm_avx512_func_to_gmp_integer<in_num_limbs, out_max_num_limbs>(       \
      uint64 in_sign, const uint64 *in_data, uint64 *out_data) {                \
    return asm_avx512_func_to_gmp_integer_##in_num_limbs##_##out_max_num_limbs( \
        in_sign, in_data, out_data);                                            \
  }
...
#ifdef COMPILE_ASM
    void compile_asm_avx512_to_gmp_integer(int in_num_limbs,
                                           int out_max_num_limbs) {
  EXPAND_MACROS_SCOPE;

  asm_function c_func(
      str("to_gmp_integer_#_#", in_num_limbs, out_max_num_limbs), 3, 9, false,
      false);
  reg_alloc regs = c_func.regs;
...
template <int in_a_num_limbs, int in_b_num_limbs, int out_num_limbs>
uint64 asm_avx512_func_add(uint64 in_a_sign, const uint64 *in_a_data,
                           uint64 in_b_sign, const uint64 *in_b_data,
                           uint64 *out_data);

#define declare_asm_avx512_func_add(in_a_num_limbs, in_b_num_limbs,                   \
                                    out_num_limbs)                                    \
  extern "C" uint64                                                                   \
      asm_avx512_func_add_##in_a_num_limbs##_##in_b_num_limbs##_##out_num_limbs(      \
          uint64 in_a_sign, const uint64 *in_a_data, uint64 in_b_sign,                \
          const uint64 *in_b_data, uint64 *out_data);                                 \
  template <>                                                                         \
  uint64 asm_avx512_func_add<in_a_num_limbs, in_b_num_limbs, out_num_limbs>(          \
      uint64 in_a_sign, const uint64 *in_a_data, uint64 in_b_sign,                    \
      const uint64 *in_b_data, uint64 *out_data) {                                    \
...
#ifdef COMPILE_ASM
    void compile_asm_avx512_add(int in_a_num_limbs, int in_b_num_limbs,
                                int out_num_limbs) {
  EXPAND_MACROS_SCOPE;

  asm_function c_func(
      str("add_#_#_#", in_a_num_limbs, in_b_num_limbs, out_num_limbs), 5, 9,
      false, false);
  reg_alloc regs = c_func.regs;
...
template <int in_a_num_limbs, int in_b_num_limbs, int out_num_limbs>
uint64 asm_avx512_func_multiply(uint64 in_a_sign, const uint64 *in_a_data,
                                uint64 in_b_sign, const uint64 *in_b_data,
                                uint64 *out_data);

#define declare_asm_avx512_func_multiply(in_a_num_limbs, in_b_num_limbs,                   \
                                         out_num_limbs)                                    \
  extern "C" uint64                                                                        \
      asm_avx512_func_multiply_##in_a_num_limbs##_##in_b_num_limbs##_##out_num_limbs(      \
          uint64 in_a_sign, const uint64 *in_a_data, uint64 in_b_sign,                     \
          const uint64 *in_b_data, uint64 *out_data);                                      \
  template <>                                                                              \
  uint64                                                                                   \
  asm_avx512_func_multiply<in_a_num_limbs, in_b_num_limbs, out_num_limbs>(                 \
      uint64 in_a_sign, const uint64 *in_a_data, uint64 in_b_sign,                         \
      const uint64 *in_b_data, uint64 *out_data) {                                         \
...
#ifdef COMPILE_ASM
    void compile_asm_avx512_multiply(int in_a_num_limbs, int in_b_num_limbs,
                                     int out_num_limbs) {
  EXPAND_MACROS_SCOPE;

  asm_function c_func(
      str("multiply_#_#_#", in_a_num_limbs, in_b_num_limbs, out_num_limbs), 5,
      9, false, false);
  reg_alloc regs = c_func.regs;
...
void compile_asm_avx512(std::string filename) {
  for_each_asm_avx512_func_to_avx512_integer(
      compile_asm_avx512_to_avx512_integer)
