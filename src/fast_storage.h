...
public:
  FastStorage(FastAlgorithmCallback *weso) {
    ...
    for (int i = 0; i < intermediates_threads; i++) {
      storage_threads.push_back(
          std::thread([=] { CalculateIntermediatesThread(); }));
    }
  }
  ...
  void CalculateIntermediatesInner(form y, uint64_t iter_begin) {
    ...
    for (uint64_t iteration = iter_begin; iteration < iter_begin + (1 << 15);
         iteration++) {
  ...
  uint64_t GetFinishedSegment() {
    while (intermediates_stored[2 * (intermediates_iter / (1 << 16))] == true &&
           intermediates_stored[2 * (intermediates_iter / (1 << 16)) + 1] ==
               true) {
    ...
    while (!stopped) {
      {
        ...
        intermediates_cv.wait(
            lk, [&] { return (!pending_intermediates.empty() || stopped); });
        if (!stopped) {
