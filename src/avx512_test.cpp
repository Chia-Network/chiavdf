...
template <class intnx, class avx512_intnx, class intjx, class avx512_intjx>
void do_benchmark(string name, int num_bits) {
...
#define DO_BENCH(x)                                                            \
  ...
  for (int i = 0; i < num_iterations_outer; ++i) {
    {
      TRACK_CYCLES_NAMED(name + "_nothing");
      DO_BENCH()
    }
    {
      TRACK_CYCLES_NAMED(name + "_to_avx512");
      DO_BENCH(a_avx512 = a;)
    }
    {
      TRACK_CYCLES_NAMED(name + "_to_avx512");
    ...
    {
      TRACK_CYCLES_NAMED(name + "_add_avx512");
      DO_BENCH(c_avx512.set_add(a_avx512, b_avx512);)
    }
    {
      TRACK_CYCLES_NAMED(name + "_add_to_gmp");
      DO_BENCH(c_avx512.assign(c);)
    }
    {
      TRACK_CYCLES_NAMED(name + "_add_gmp");
      DO_BENCH(c.set_add(a, b);)
    }
    {
      TRACK_CYCLES_NAMED(name + "_mul_avx512");
      DO_BENCH(d_avx512.set_mul(a_avx512, b_avx512);)
    }
    {
      TRACK_CYCLES_NAMED(name + "_mul_avx512_gmp");
      DO_BENCH(a_avx512 = a; b_avx512 = b; d_avx512.set_mul(a_avx512, b_avx512);
               d_avx512.assign(d);)
    }
    {
      TRACK_CYCLES_NAMED(name + "_mul_gmp");
      DO_BENCH(d.set_mul(a, b);)
    }
    {
      TRACK_CYCLES_NAMED(name + "_mul_to_gmp");
      DO_BENCH(d_avx512.assign(d);)
    }
  }
...
/*template<class intnx, class avx512_intnx> void load_int(const char* num_bits,
avx512_intnx& out) { int num_bits_int=from_string<int>(num_bits); integer
test_int=rand_integer((num_bits_int<0)? -num_bits_int : num_bits_int);
    //integer test_int=integer(1)<<((num_bits_int<0)? -num_bits_int :
num_bits_int); if (num_bits_int<0) { test_int=-test_int;
    }
    intnx test;
