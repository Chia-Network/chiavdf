...
int64 abs_int(int64 v) { return (v < 0) ? -v : v; }
...
// note: this floors the quotient instead of truncating it like the div
// instruction
int64 divide_table(int64 a, int64 b, int64 &q) {
  ...
  bool invalid_1 =
      (int128(qb_64) != qb_128 || int128(r_64) != r_128 || uint64(r_64) >= b);
...
void gcd_64(array<int64, 2> start_a,
            pair<array<int64, 4>, array<int64, 2>> &res, int &num_iterations,
            bool approximate, int max_iterations) {
  const bool test_asm_funcs = false;
  ...
  while (true) {
    ...
    if (approximate) {
      ...
      if (is_even) {
        valid_exact =
            (new_a[1] >= -new_uv[1 * 2 + 0] &&
             new_a[0] - new_a[1] >= new_uv[1 * 2 + 1] - new_uv[0 * 2 + 1]);
      } else {
        valid_exact =
            (new_a[1] >= -new_uv[1 * 2 + 1] &&
             new_a[0] - new_a[1] >= new_uv[1 * 2 + 0] - new_uv[0 * 2 + 0]);
      }

      // valid=valid_exact;
      valid = (new_a[1] >= -new_uv[1 * 2 + 0] &&
               new_a[0] - new_a[1] >= new_uv[1 * 2 + 1] - new_uv[0 * 2 + 1]) &&
              (new_a[1] >= -new_uv[1 * 2 + 1] &&
               new_a[0] - new_a[1] >= new_uv[1 * 2 + 0] - new_uv[0 * 2 + 0]);
  ...
  if (test_asm_funcs) {
    assert(uv == uv_asm);
    // assert(a==a_asm); the asm code will update a even if it becomes invalid;
    // fine since it's not used
    assert(num_iterations == asm_num_iterations);
  }
