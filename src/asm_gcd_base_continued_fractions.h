...
// clobbers v
void range_check(reg_vector v, reg_vector range, reg_vector c_double_abs_mask,
  ...
  // todo //can replace this with POR into an accumulator then use a single
  // PTEST todo //can compile the code twice for is_leher being true and false,
  // then branch to the appropriate version todo //can probably get rid of the
  // uv range checks if is_lehmer is true todo //can get rid of the ab range
  // checks if the table is used and each table uv value has a magnitude less
...
// clobbers b
// this calculates the dot product of each lane separately and puts the result
// in that lane
void dot_product_exact(array<reg_vector, 2> a, array<reg_vector, 2> b,
                       reg_vector v, reg_vector range,
                       reg_vector c_double_abs_mask, string out_of_range_label,
  ...
  // todo //for avx, can get rid of a lot of the MOVs by using the 3-operand
...
// will assign u and v
void gcd_base_continued_fraction(reg_alloc regs, reg_vector ab, reg_vector u,
                                 reg_vector v, reg_vector is_lehmer,
                                 reg_vector ab_threshold,
  ...
  reg_scalar c_table_delta_minus_1 =
      regs.bind_scalar(m, "c_table_delta_minus_1");
  APPEND_M(str("MOV `c_table_delta_minus_1, #",
               constant_address_uint64(c_table.delta - 1, c_table.delta - 1)));
  ...
  APPEND_M(str("MOVAPD `range_check_range, #",
               constant_address_double(range_check_range, range_check_range)));
  APPEND_M(str("MOVAPD `double_abs_mask, #",
               constant_address_uint64(double_abs_mask, double_abs_mask)));
...

  {
    ...
    // if (q_scalar<0 ||
    // q_scalar>=(c_table.range_end_shifted-c_table.range_start_shifted)<<5)
    // goto no_table_label
    // this bypasses the "ab[1]<=ab_threshold" check so we need to do it again
    // in no_table_label
    APPEND_M(str("SUB `q_scalar, #", to_hex(c_table.range_start_shifted << 5)));
    APPEND_M(
        str("JB #", track_asm("gcd_base below table start", no_table_label)));
    APPEND_M(
        str("CMP `q_scalar, #",
            to_hex((c_table.range_end_shifted - c_table.range_start_shifted)
                   << 5)));
    APPEND_M(
        str("JAE #", track_asm("gcd_base after table end", no_table_label)));
...
#else
    APPEND_M(str("MOVAPD `m_0, [") + asmprefix +
             str("gcd_base_table+`q_scalar]"));
    APPEND_M(str("MOVAPD `m_1, [") + asmprefix +
             str("gcd_base_table+16+`q_scalar]"));
#endif
    ...
    APPEND_M(str("MOVAPD `tmp, `ab"));
    APPEND_M(str(
        "CMPLEPD `tmp, `ab_threshold")); // tmp all 0s if (ab[0]>ab_threshold[0]
                                         // && ab[1]>ab_threshold[1])
    APPEND_M(str("PTEST `tmp, `tmp"));
    APPEND_M(
        str("JNZ #", track_asm("gcd_base ab[1]<=ab_threshold", exit_label)));

    // if ( (q_scalar_2&(c_table.delta-1))==0 ||
    // (q_scalar_2&(c_table.delta-1))==c_table.delta-1 ) goto no_table_label
    APPEND_M(str("AND `q_scalar_2, `c_table_delta_minus_1"));
    APPEND_M(
        str("JZ #", track_asm("gcd_base on slot boundary", no_table_label)));
    APPEND_M(str("CMP `q_scalar_2, `c_table_delta_minus_1"));
    APPEND_M(
        str("JE #", track_asm("gcd_base on slot boundary", no_table_label)));
    ...
    auto calculate_using_m = [&](string fail_label) {
      ...
      dot_product_exact({m_0, m_1}, {tmp, tmp2}, new_ab, c_range_check_range,
                        c_double_abs_mask,
      ...
      APPEND_M(str(
          "SHUFPD `new_ab_1, `new_ab_1, 3")); // new_ab_1=<new_ab[1], new_ab[1]>
      ...
      dot_product_exact(
          {m_0, m_1}, {tmp, tmp2}, new_u, c_range_check_range,
          c_double_abs_mask,
          track_asm("gcd_base uv range check failed", fail_label));

      // todo //for avx, can replace some shuffles with broadcasts. can make a
      // macro that expands to the proper instructions
      ...
      dot_product_exact(
          {m_0, m_1}, {tmp, tmp2}, new_v, c_range_check_range,
          c_double_abs_mask,
          track_asm("gcd_base uv range check failed", fail_label));
    };
    ...
    APPEND_M(
        str("JBE #", track_asm("gcd_base new_ab[0]<=ab_threshold for table",
                               no_table_label)));
    ...
    APPEND_M("#gcd_base no table");
    {
      ...
      // have to do this check here because it might have been skipped: if
      // (ab[1]<=ab_threshold) goto exit_label
      APPEND_M(str("MOVAPD `tmp, `ab"));
      APPEND_M(str("CMPLEPD `tmp, `ab_threshold")); // tmp all 0s if
                                                    // (ab[0]>ab_threshold[0] &&
                                                    // ab[1]>ab_threshold[1])
      APPEND_M(str("PTEST `tmp, `tmp"));
      APPEND_M(
      ...
      // q=floor(q);
      // this requires SSE4. if not present, can also add and subtract a magic
      // number
    ...
    // tmp all 0s if (ab_delta[0]+new_uv_0[0]>=new_uv_1[0] &&
    // ab_delta[1]+new_uv_0[1]>=new_uv_1[1])
    APPEND_M(str("CMPLTPD `tmp, `m_1"));

    // m_1=-new_uv_1
    APPEND_M(str("XORPD `m_1, #",
                 constant_address_uint64(double_sign_mask, double_sign_mask)));

    // new_ab_1 all 0s if (new_ab_1[0]>=-new_uv_1[0] &&
    // new_ab_1[1]>=-new_uv_1[1])
    APPEND_M(str("CMPLTPD `new_ab_1, `m_1"));

    // if (is_lehmer && !(ab_delta[0]+new_uv_0[0]>=new_uv_1[0] &&
    // ab_delta[1]+new_uv_0[1]>=new_uv_1[1])) goto exit_label if (is_lehmer &&
    // !(new_ab_1[0]>=-new_uv_1[0] && new_ab_1[1]>=-new_uv_1[1])) goto
    // exit_label
    APPEND_M(str("ORPD `tmp, `new_ab_1"));   // tmp all 0s if passed is true
    APPEND_M(str("ANDPD `tmp, `is_lehmer")); // tmp all 0s if
                                             // passed||(!is_lehmer) is true
    APPEND_M(str("PTEST `tmp, `tmp"));
