template <int size> struct fixed_gcd_res {
  ...
  fixed_integer<uint64, size>
      gcd_2; // unsigned; final value of b. this is 0 for a normal gcd
...
template <int size>
fixed_gcd_res<size>
gcd(fixed_integer<uint64, size> a_signed, fixed_integer<uint64, size> b_signed,
    fixed_integer<uint64, size> threshold, bool calculate_u) {
  ...
  // if the parity is -1, negate the parity and negate the result u/v values.
  // the parity is now 1 for u, u0 is positive and u1 is negative for v, v0 is
  // negative and u1 is positive
  if (calculate_u) {
    res.s = uv[0];
