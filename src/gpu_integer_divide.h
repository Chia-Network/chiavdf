...
void normalize_divisor(fixed_integer<type, size> &b, int &shift_limbs,
                       int &shift_bits) {
  shift_limbs = 0;
  // todo //make this a variable shift (could have done it on the gpu through
  // shared memory; oh well)
...
uint32 calculate_quotient(uint32 high, uint32 low, uint64 reciprocal,
                          uint32 b) {
...
uint64 calculate_quotient(uint64 high, uint64 low,
                          fixed_integer<uint64, 2> reciprocal, uint64 b);

// should pad a by 1 limb then left shift it by num_bits
// all integers are unsigned
template <class type, int size_a, int size_b>
void divide_integers_impl(fixed_integer<type, size_a> a,
                          fixed_integer<type, size_b> b, int b_shift_limbs,
                          fixed_integer<type, size_a - 1> &q,
                          fixed_integer<type, size_b> &r) {
  const int max_quotient_size = size_a - 1;
  fixed_integer<type, max_quotient_size> res;

  auto reciprocal =
      calculate_reciprocal(b[size_b - 1], (size_b >= 2) ? b[size_b - 2] : 0);

  fixed_integer<type, size_a> b_shifted;
  b_shifted = b;
  b_shifted.left_shift_limbs(size_a - size_b -
                             1); // it is already left shifted by b_shift_limbs

  int quotient_size = size_a - (size_b - b_shift_limbs);

  for (int x = 0; x < max_quotient_size; ++x) {
    // this is more efficient than having an if statement without a break
    // because of the compiler
    if (x >= quotient_size) {
      break;
    }
    {
      type qj = calculate_quotient(a[size_a - 1 - x], a[size_a - 2 - x],
                                   reciprocal, b[size_b - 1]);

      // this is slower than using the doubles even though the doubles waste
      // half the registers ptxas generates horrible code which isn't scheduled
      // properly uint64 qj_64=((uint64(a[size_a-1-x])<<32) |
      // uint64(a[size_a-2-x])) / uint64(b[size_b-1]); uint32 qj=uint32(min(
      // qj_64, uint64(~uint32(0)) ));
...
// these are signed
// this has a bug if size_a<size_b and the quotient is nonzero. remainder is
// wrong. dont care
template <class type, int size_a, int size_b>
void divide_integers(const fixed_integer<type, size_a> &a,
                     const fixed_integer<type, size_b> &b,
...
template <class type, int size_a, int size_b>
fixed_integer<type, size_a> operator/(const fixed_integer<type, size_a> &a,
                                      const fixed_integer<type, size_b> &b) {
  fixed_integer<type, size_a> q;
  fixed_integer<type, size_b> r;
...
template <class type, int size_a, int size_b>
fixed_integer<type, size_b> operator%(const fixed_integer<type, size_a> &a,
                                      fixed_integer<type, size_b> b) {
  fixed_integer<type, size_a> q;
  fixed_integer<type, size_b> r;
...
fixed_integer<uint64, 2> calculate_reciprocal(uint64 high, uint64 low) {
  ...
  return fixed_integer<uint64, 2>(
      to_uint64(a / b + fixed_integer<uint32, 6>(integer(1))) << 31);
}

// result is >= the actual reciprocal. it is approximately 2^127/((HIGH |
// LOW)/2^127) the max value is 2^127 + 2^31
/*fixed_integer<uint64, 2> calculate_reciprocal(uint64 high, uint64 low) {
    assert((high>>63)!=0); //normalized
    ...
    return fixed_integer<uint64, 2>(to_uint64(a/b + fixed_integer<uint32,
6>(integer(1)))<<31);
} */
...
uint64 calculate_quotient(uint64 high, uint64 low,
  ...
  //
  // max value of the product is (2^128-1)*(2^127 + 2^31) = 2^255 + 2^159 -
  // 2^127 - 2^31
  ...
  // this can't overflow because the max value of the product has e.g. bit 254
  // cleared
