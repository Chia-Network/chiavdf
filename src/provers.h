...
public:
  OneWesolowskiProver(Segment segm, integer D, form *intermediates,
  ...
  void start() { GenerateProof(); }

  void stop() {}

  bool PerformExtraStep() { return !stop_signal; }

  void OnFinish() { is_finished = true; }
...
public:
  TwoWesolowskiProver(Segment segm, integer D, TwoWesolowskiCallback *weso,
                      std::atomic<bool> &stop_signal)
      : Prover(segm, D), stop_signal(stop_signal) {
    this->weso = weso;
...
public:
  InterruptableProver(Segment segm, integer D, FastAlgorithmCallback *weso)
      : Prover(segm, D) {
    this->weso = weso;
    this->done_iterations = segm.start;
  ...
  bool IsRunning() { return !is_paused; }

  bool IsFullyFinished() { return is_fully_finished; }
