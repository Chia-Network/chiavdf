...
const int track_cycles_array_size =
    track_cycles_max_num * track_cycles_num_buckets;
...
void track_cycles_init() {
  ...
  if (!is_init) {
    ...
    // track_cycles_call_counters=new
    // uint64[track_cycles_max_num*track_cycles_num_buckets];
...
void track_cycles_output_stats() {
  ...
  for (int x = 0; x < track_cycles_next_slot; ++x) {
    ...
    for (int y = 0; y < track_cycles_num_buckets; ++y) {
      total_calls +=
          track_cycles_call_counters.at(x * track_cycles_num_buckets + y);
    }
    ...
    for (int y = 0; y < track_cycles_num_buckets; ++y) {
      double cycles =
          track_cycles_cycle_counters.at(x * track_cycles_num_buckets + y);
      double calls =
          track_cycles_call_counters.at(x * track_cycles_num_buckets + y);
...
struct track_cycles_impl {
  ...
  ~track_cycles_impl() {
    ...
    assert(index >= 0 &&
           index < track_cycles_max_num * track_cycles_num_buckets);
...
template <int expected_size_out, int padded_size_out, int expected_size_a,
          int padded_size_a, int expected_size_b, int padded_size_b>
void mpz_impl_set_mul(mpz<expected_size_out, padded_size_out> &out,
...
template <int d_expected_size, int d_padded_size> struct alignas(64) mpz {
  ...
  // int mpz._mp_size: abs(_mp_size) is number of limbs in use; 0 if the integer
  // is zero. it is negated if the integer is negative mp_limb_t* mpz._mp_d:
  // pointer to limbs do not call mpz_swap on this. mpz_swap can be called on
  // other gmp integers
  mpz_struct c_mpz;
  ...
  bool was_reallocated() const { return c_mpz._mp_d != data; }
  ...
  ~mpz() {
    ...
    // if c_mpz.data wasn't reallocated, it has to point to this instance's data
    // and not some other instance's data if mpz_swap was used, this might be
    // violated
  ...
  void set_add(const mpz_struct *a, uint64 b) { mpz_add_ui(*this, a, b); }
  ...
  void set_sub(const mpz_struct *a, uint64 b) { mpz_sub_ui(*this, a, b); }
  ...
  template <int expected_size_a, int padded_size_a, int expected_size_b,
            int padded_size_b>
  void set_mul(const mpz<expected_size_a, padded_size_a> &a,
               const mpz<expected_size_b, padded_size_b> &b) {
    mpz_impl_set_mul(*this, a, b);
  }

  void set_mul(const mpz_struct *a, int64 b) { mpz_mul_si(*this, a, b); }

  void set_mul(const mpz_struct *a, uint64 b) { mpz_mul_ui(*this, a, b); }
  ...
  void negate() { mpz_neg(*this, *this); }

  void abs() { mpz_abs(*this, *this); }
  ...
  int compare_abs(const mpz_struct *t) const { return mpz_cmpabs(*this, t); }

  int compare_abs(uint64 t) const { return mpz_cmpabs_ui(*this, t); }

  // returns 0 if *this==0
  int sgn() const { return mpz_sgn(_()); }

  int num_bits() const { return mpz_sizeinbase(*this, 2); }

  // 0 if this is 0
  int num_limbs() const { return mpz_size(*this); }

  const uint64 *read_limbs() const { return (uint64 *)mpz_limbs_read(*this); }

  // limbs are uninitialized. call finish
  uint64 *write_limbs(int num) { return (uint64 *)mpz_limbs_write(*this, num); }
  ...
  // it can be less than that as long as it is at least the number of nonzero
  // limbs it can be 0 if the result is 0
...
struct alignas(64) thread_counter {
  std::atomic<uint64> counter_value{
      0}; // updated atomically since only one thread can write to it
  ...
  thread_counter() { assert((uint64(this) & 63) == 0); }
};
...
struct thread_state {
  ...
  uint64 v() { return this_counter().counter_value; }
  ...
  bool fence_absolute(uint64 t_v) {
    ...
    while (other_counter().counter_value < t_v) {
      if (this_counter().error_flag.load() ||
          other_counter().error_flag.load()) {
        raise_error();
        break;
      }
  ...
  bool has_error() { return this_counter().error_flag; }
...
struct alignas(64) gcd_uv_entry {
  ...
  template <class mpz_type>
  void matrix_multiply(const mpz_type &in_a, const mpz_type &in_b,
                       mpz_type &out_a, mpz_type &out_b) const {
...
template <class mpz_type> struct alignas(64) gcd_results_type {
  ...
  static const int num_counter =
      gcd_max_iterations + 1; // one per outputted entry

  array<gcd_uv_entry, gcd_max_iterations + 1> uv_entries;

  int end_index = 0;

  mpz_type &get_a_start() { return as[0]; }

  mpz_type &get_b_start() { return bs[0]; }
  ...
  // this will increase the counter value and wait until the result at index is
  // available index 0 only has exit_flag initialized
  bool get_entry(int counter_start_delta, int index,
                 const gcd_uv_entry **res) const {
    *res = nullptr;
...
template <class mpz_type>
bool gcd_unsigned(int counter_start_delta,
                  gcd_results_type<mpz_type> &c_results,
                  const array<uint64, gcd_size> &threshold) {
  ...
  data.uv_counter_start =
      c_thread_state.counter_start + counter_start_delta + 1;
  // TODO: come up with something better here
  data.out_uv_counter_addr = reinterpret_cast<uint64_t *>(
      &(c_thread_state.this_counter().counter_value));
  data.out_uv_addr = (uint64 *)&(c_results.uv_entries[1]);
  data.iter = -1;
  data.a_end_index = (a_limbs == 0) ? 0 : a_limbs - 1;

  if (is_vdf_test) {
    assert((uint64(data.out_uv_addr) & 63) == 0); // should be cache line
                                                  // aligned
  }

  int error_code = hasAVX2() ? asm_code::asm_avx2_func_gcd_unsigned(&data)
  ...
  assert(data.iter >= 0 &&
         data.iter <=
             gcd_max_iterations); // total number of iterations performed
  bool is_even =
      ((data.iter - 1) & 1) == 0; // parity of last iteration (can be -1)
  ...
  if (!c_thread_state.advance(counter_start_delta +
                              gcd_results_type<mpz_type>::num_counter)) {
    return false;
  }
