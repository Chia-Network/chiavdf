...
matrix2 identity_matrix() { return {vector2{1, 0}, vector2{0, 1}}; }

matrix2 quotient_matrix(double q) {
  assert(int64(q) == q);

  return {vector2{0, 1}, vector2{1, -q}};
}
...
bool range_check(double v) {
  ...
  // if two floats are added/subtracted and there is a loss of precision, the
  // absolute value of the result will be greater than this same with
  // multiplication and fma (all of the doubles are integers whether they are
  //exact or not)
  return abs(v) <= double((1ull << 53) - 1);
}

bool dot_product_exact(vector2 a, vector2 b, double &v,
                       bool result_always_in_range = false) {
  v = a[0] * b[0];
  if (!range_check(v)) {
    return false;
  }
  ...
  if (result_always_in_range) {
    // still need the first range_check since the intermediate value might not
    // be in range
...
bool dot_product_exact(array<uint64, 2> a, array<uint64, 2> b, uint64 &v,
                       bool result_always_in_range = false) {
  uint64 t1;
...
template <class type>
bool multiply_exact(array<array<type, 2>, 2> a, array<type, 2> b,
                    array<type, 2> &v, bool result_always_in_range = false) {
  return dot_product_exact(a[0], b, v[0], result_always_in_range) &&
         dot_product_exact(a[1], b, v[1], result_always_in_range);
}

template <class type>
bool multiply_exact(array<array<type, 2>, 2> a, array<array<type, 2>, 2> b,
                    array<array<type, 2>, 2> &v,
                    bool result_always_in_range = false) {
  return dot_product_exact(a[0], array<type, 2>{b[0][0], b[1][0]}, v[0][0],
                           result_always_in_range) &&
         dot_product_exact(a[0], array<type, 2>{b[0][1], b[1][1]}, v[0][1],
                           result_always_in_range) &&
         dot_product_exact(a[1], array<type, 2>{b[0][0], b[1][0]}, v[1][0],
                           result_always_in_range) &&
         dot_product_exact(a[1], array<type, 2>{b[0][1], b[1][1]}, v[1][1],
                           result_always_in_range);
}
...
struct continued_fraction {
  ...
  bool truncate(double max_matrix_value) {
    ...
    while (true) {
      ...
      double max_value =
          max(max(abs(m[0][0]), abs(m[0][1])), max(abs(m[1][0]), abs(m[1][1])));
  ...
  pair<double, double> get_bound() {
    ...
    if (!a_exact) {
      // if a isn't exact, the next double value after a is inside the continued
      // fraction (since it got rounded down). this assumes
      //  the bound isn't so small that it is close to the double machine
      //  epsilon; this is checked later by the double_table code
      // if a is exact then it is inside the continued fraction
      a = nextafter(a, HUGE_VAL);
    }

    // if b isn't exact, then it got rounded down and the b value is inside the
    // continued fraction. the next value after b will
    //  be outside the continued fraction
    // if b is exact then it is also inside the continued fraction and the next
    // value is outside
...
// if you add 1 to the integer representation of a positive double, it will
// increase the value by 1 machine epsilon (assuming no overflow)
template <class type> struct double_table {
  vector<type>
      data; // data[x] is >= range_start+x*delta and < range_start+(x+1)*delta
  ...
  double_table(int t_exponent_bits, int t_fraction_bits) {
    ...
    delta_bits.fraction = 1ull
                          << (double_bits::fraction_num_bits - fraction_bits);
    delta = delta_bits.to_uint64();

    assert(range_end > range_start);
    assert(range_start % delta == 0);
    assert(range_end % delta == 0);
    assert((range_end - range_start) / delta ==
           1ull << (exponent_bits + fraction_bits));
  ...
  bool lookup(double v, type &res) {
    ...
    if (v_bits_shifted < range_start_shifted ||
        v_bits_shifted >= range_end_shifted) {
      return false;
    }

    // the table doesn't work if v is exactly between two slots
    // happens if the remainder is 0 for one of the quotients
    if ((v_bits & (delta - 1)) == 0 || (v_bits & (delta - 1)) == delta - 1) {
      return false;
    }
  ...
  // returns true if the range is at least 0.5 entries wide (for that area of
  // the table) and is within the table bounds
  bool assign(pair<double, double> range, type value,
              vector<type> &old_values) {
    old_values.clear();

    double start_double = range.first;
    double end_double = range.second;

    assert(start_double > 0 && end_double > 0 && end_double >= start_double &&
           isfinite(start_double) && isfinite(end_double));
    ...
    for (uint64 pos = start_pos; pos < end_pos; ++pos) {
      ...
      // if start_double==slot_range.first, then both ranges have the same
      // starting double so that's fine if end_double==slot_range.second, then
      // both ranges have the same ending double which is also fine
...
bool add_to_table(double_table<continued_fraction> &c_table,
...
void add_children_to_table(double_table<continued_fraction> &c_table,
                           continued_fraction f) {
...
double_table<continued_fraction>
generate_table(int exponent_bits, int fraction_bits,
               uint64 truncate_max_value = 1ull << 53,
               bool output_stats = false, bool dump = false) {
  ...
  // if the exponent has too many bits, some of the table entries will span
  // multiple integers and won't have any entries all of the full entries are at
  // the start of the table, and all of the empty entires are at the end. they
  // aren't interleaved when setting up the table range checks, should truncate
  // off all of the empty values so they won't affect cache coherency
  int num_empty = 0;

  for (int x = 0; x < c_table.data.size(); ++x) {
    if (dump) {
      cerr << c_table.index_range(x).first << ", "
           << c_table.index_range(x).second << " : ";
...
//-to calculate the next uv, just multiply the unsigned matricies together. also
//add the parities modulo 2 -to calculate ab from the starting ab, do a
//subtraction in the dot product instead of adding, then take the absolute value
//of the
//  result. can also use the parity to decide what way to do the subtraction.
//  - odd parity: b-a, a-b
//  -even parity: a-b, b-a
//  -can calculate assuming even parity. then sign extend the parity to 64 bits
//  (from 1 bit) and use the parity as the carry in,
//   then xor the result by the sign extended parity and add the carry. this can
//   also determine the parity if it is unknown
//
//   odd parity uv: { <=0 > 0
//                    > 0 < 0}
//  even parity uv: { >=0 <=0s
...
// if this returns false then the new values are invalid and the old values are
// valid this works if u/v are unsigned, if v[1]-v[0] is replaced with
// |v[1]|+|v[0]| and -u[1] is replaced with |u1| etc
bool check_lehmer(array<int64, 2> a, array<int64, 2> u, array<int64, 2> v) {
  ...
  return a[1] >= -u[1] &&
         int128(a[0]) - int128(a[1]) >=
             int128(v[1]) - int128(v[0]) && // even parity
         a[1] >= -v[1] &&
         int128(a[0]) - int128(a[1]) >=
             int128(u[1]) - int128(u[0]) //  odd parity
      ;
}

bool gcd_base_continued_fraction(vector2 &ab, matrix2 &uv, bool is_lehmer,
                                 double ab_threshold = 0) {
  static double_table<continued_fraction> c_table =
      generate_table(gcd_table_num_exponent_bits, gcd_table_num_fraction_bits);
  ...
  while (true) {
    if (debug_output)
      print("======== 1:", iter, ab[1], ab_threshold);
    ...
    if (enable_table && c_table.lookup(q, f)) {
      assert(
          !f.values.empty()); // table should be set up not to have empty values

      if (debug_output)
        print("3:", f.get_matrix()[0][0], f.get_matrix()[1][0],
              f.get_matrix()[0][1], f.get_matrix()[1][1]);

      bool new_ab_valid =
          multiply_exact(f.get_matrix(), ab, new_ab,
                         true); // a and b can only be reduced in magnitude
      bool new_uv_valid = multiply_exact(f.get_matrix(), uv, new_uv);
      bool new_a_valid = (new_ab[0] > ab_threshold);

      if (debug_output)
        print("4:", new_ab_valid, new_uv_valid, new_a_valid);
      if (debug_output)
        print("5:", new_ab[0], new_ab[1], new_uv[0][0], new_uv[1][0],
              new_uv[0][1], new_uv[1][1]);

      if (new_ab_valid && new_uv_valid && new_a_valid) {
        used_table = true;
        ++iter_table;
      } else {
        // this should be disabled to make the output the same as the non-table
        // version this is disabled in the asm version if (is_lehmer &&
        // ab_threshold==0) { can also bypass the table but it is probably
        // slower if ab_threshold is not 0, need to keep going since the partial
        // gcd is about to terminate break;
        //}
      }
    }

    if (!used_table) {
      // the native instruction is as fast as adding then subtracting some magic
      // number
      q = floor(q);
      ++iter_slow;
      ...
      if (debug_output)
        print("6.5:", new_ab[0], new_ab[1], new_uv[0][0], new_uv[1][0],
              new_uv[0][1], new_uv[1][1]);
    ...
    // this has to be checked on the first iteration if the table is not used
    // (since there could be a giant quotient e.g. a=b) will check it even if
    // the table is used. shouldn't affect performance
    if (is_lehmer) {
      double ab_delta = new_ab[0] - new_ab[1];
      assert(range_check(ab_delta)); // both are nonnegative so the subtraction
                                     // can't increase the magnitude
      assert(ab_delta >= 0); // ab[0] has to be greater

      // the magnitudes add for these
      // however, the comparison is ab_delta >= u_delta or v_delta, and
      // ab_delta>=0, so the values of u_delta and v_delta can
      //  be increased. if the calculation is not exact, the values will be
      //  ceil'ed so they are exact or increased; never reduced
      // double u_delta=uv[1][0]-uv[0][0];
      // double v_delta=uv[1][1]-uv[0][1];

      // even parity:
      // don't care what the result of the odd comparison is as far as
      // correctness goes. for performance, it has to be true most
      //  of the time
      //  uv[0][1]<=0 ; uv[1][1]>=0
      // ab_delta+uv[0][1] is exact because the signs are opposite
      // ab_delta+uv[0][0] is <= the true value so the comparison might return
      // false wrongly. should be fine
      ...
      bool passed =
          new_ab[1] >= -new_uv[1][0] &&
          ab_delta + new_uv[0][1] >=
              new_uv[1][1] && // even parity. for odd parity this is always true
          new_ab[1] >= -new_uv[1][1] &&
          ab_delta + new_uv[0][0] >=
              new_uv[1][0] //  odd parity. for even parity this is always true
          ;

      if (debug_output)
        print("8:", new_ab[1], new_uv[1][0], ab_delta, new_uv[0][1],
              new_uv[1][1]);
      if (debug_output)
        print("9:", new_ab[1], new_uv[1][1], ab_delta, new_uv[0][0],
              new_uv[1][0]);
      if (debug_output)
        print("10:", passed);
    ...
    // print( "foo" );
    {
      ...
      int64 a_expected = int64(uv[0][0]) * int64(ab_start[0]) +
                         int64(uv[0][1]) * int64(ab_start[1]);
      int64 b_expected = int64(uv[1][0]) * int64(ab_start[0]) +
                         int64(uv[1][1]) * int64(ab_start[1]);
  ...
  if (test_asm_run) {
    ...
    double asm_u[2];
    double asm_v[2];
    uint64 asm_is_lehmer[2] = {(is_lehmer) ? ~0ull : 0ull,
                               (is_lehmer) ? ~0ull : 0ull};
    double asm_ab_threshold[2] = {ab_threshold, ab_threshold};
    uint64 asm_no_progress;
    int error_code =
        hasAVX2()
            ? asm_code::asm_avx2_func_gcd_base(asm_ab, asm_u, asm_v,
                                               asm_is_lehmer, asm_ab_threshold,
                                               &asm_no_progress)
            : asm_code::asm_cel_func_gcd_base(asm_ab, asm_u, asm_v,
                                              asm_is_lehmer, asm_ab_threshold,
                                              &asm_no_progress);
...
bool gcd_base_continued_fraction_2(vector2 &ab_double, matrix2 &uv_double,
                                   bool is_lehmer,
                                   double ab_threshold_double = 0) {
  ...
  while (ab[1] > threshold_int) {
    ...
    if (int64(ab_double_new[0]) != new_ab[0] ||
