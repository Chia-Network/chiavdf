...
/*struct generic_stats {
    ...
    void output(string name) {
        ...
        vector<double> percentiles={0, 0.01, 0.1, 1, 10, 25, 50, 75, 90,
99, 99.9, 99.99, 100};
...
struct track_max_type {
  ...
  void output(int basis_bits) {
    print("== track max ==");
    for (auto c : data) {
      print(c.first.second, double(c.second.first) / basis_bits,
            c.second.second);
    }
  }
};
track_max_type track_max;
//#define TRACK_MAX(data) track_max.add(#data " {" __func__ ":" "__LINE__" ")",
//(data).num_bits())
#define TRACK_MAX(data)                                                        \
  track_max.add(__LINE__, #data, (data).num_bits(), (data) < 0)
...
static bool allow_integer_constructor =
    false; // don't want static integers because they use the wrong allocator
...
struct integer {
  ...
  bool get_bit(int index) { return mpz_tstbit(impl, index); }
  ...
  bool prime() const { return is_prime_bpsw(impl) != 0; }

  bool operator<(const integer &t) const { return mpz_cmp(impl, t.impl) < 0; }
  ...
  bool operator<(int i) const { return mpz_cmp_si(impl, i) < 0; }
  ...
  int num_bits() const { return mpz_sizeinbase(impl, 2); }
};
