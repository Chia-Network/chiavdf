...
// always works
void repeated_square_original(vdf_original &vdfo, form &f, const integer &D,
                              const integer &L, uint64 base, uint64 iterations,
...
void repeated_square(form f, const integer &D, const integer &L,
  ...
  while (!stopped) {
    ...
    uint64 actual_iterations = repeated_square_fast(
        square_state, f, D, L, num_iterations, batch_size, weso);
    ...
    if (actual_iterations < batch_size) {
      // the fast algorithm terminated prematurely for whatever reason. f is
    ...
    if (num_iterations >= last_checkpoint) {
      ...
      if (fast_algorithm) {
        if (fast_storage != NULL) {
          fast_storage->SubmitCheckpoint(
              static_cast<FastAlgorithmCallback *>(weso)->y_ret,
              last_checkpoint);
        } else if (last_checkpoint % (1 << 16) == 0) {
          // Notify prover event loop, we have a new segment with intermediates
          // stored.
          {
...
Proof ProveOneWesolowski(uint64_t iters, integer &D, form f,
...
Proof ProveTwoWeso(integer &D, form x, uint64_t iters, uint64_t done_iterations,
                   TwoWesolowskiCallback *weso, int depth,
  ...
  if (depth == 2) {
    ...
    uint64 checkpoint =
        (done_iterations + iters) - (done_iterations + iters) % 100;
    form y = *(weso->GetForm(checkpoint));
    repeated_square_original(vdfo_proof, y, D, L, 0,
                             (done_iterations + iters) % 100, NULL);
  ...
  TwoWesolowskiProver prover(sg, D, weso, stop_signal);
  prover.start();
  Proof proof2 = ProveTwoWeso(D, y1, iterations2, done_iterations + iterations1,
                              weso, depth + 1, stop_signal);
...
class ProverManager {
public:
  ProverManager(integer &D, FastAlgorithmCallback *weso,
                FastStorage *fast_storage, int segment_count,
  ...
  void start() { main_loop.emplace(&ProverManager::RunEventLoop, this); }
  ...
  Proof Prove(uint64_t iteration) {
    ...
    if (iteration % (1 << 16)) {
      // Recalculate everything from the checkpoint, since there is no guarantee
      // the iter didn't arrive late.
      integer L = root(-D, 4);
      PulmarkReducer reducer;
      std::unique_ptr<form[]> intermediates(
          new form[(iteration % (1 << 16)) / 10 + 100]);
    ...
    while (!valid_proof && !stopped) {
      ...
      for (int i = segment_count - 1; i >= 0; i--) {
        ...
        while (position < done_segments[i].size() &&
               proved_iters + segment_size <= iteration) {
      ...
      if (blobs > 63 || proved_iters < iteration - iteration % (1 << 16)) {
        std::cout
            << "Warning: Insufficient segments yet. Retrying in 1 minute\n";
    ...
    proof_serialized =
        SerializeForm(proof_segments[proof_segments.size() - 1].proof, d_bits);
    for (int i = proof_segments.size() - 2; i >= 0; i--) {
      ...
      VectorAppend(
          proof_serialized,
          GetB(D, proof_segments[i].x, proof_segments[i].y).to_bytes());
      VectorAppend(proof_serialized,
                   SerializeForm(proof_segments[i].proof, d_bits));
    }
    Proof proof(y_serialized, proof_serialized);
    ...
    std::cout << "Current weso iteration: " << vdf_iteration
              << ". Extra proof time (in VDF iterations): "
              << vdf_iteration - iteration << "\n";
    return proof;
  }
  ...
  void RunEventLoop() {
    ...
    // * atomic
    const bool multi_proc_machine =
        (std::thread::hardware_concurrency() >= 16) ? true : false;
    bool warned = false;
    ...
    while (!stopped) {
      ...
      if (vdf_iteration >= 5e8) {
        if (!increased_proving && multi_proc_machine) {
          std::cout << "Warning: VDF running longer than (expected) 5 minutes. "
                       "Adding 2 more proving threads.\n";
      ...
      bool new_last_segment = false;
      {
        ...
        if (pending_iters_last_sg.size() > 0 &&
            vdf_iteration >= *pending_iters_last_sg.begin()) {
      ...
      uint64_t best_pending_iter = (1LL << 63);
      {
        ...
        // We can advance the proving iterations only when a new 2^16 segment is
        // done.
        if (new_small_segment) {
          uint64_t expected_proving_iters = 0;
          if (done_segments[0].size() > 0) {
            expected_proving_iters =
                done_segments[0][done_segments[0].size() - 1].start +
          ...
          if (pending_iters.size() > 0 &&
              expected_proving_iters >=
                  best_pending_iter - best_pending_iter % (1 << 16)) {
            // Calculate the real number of iters we can prove.
            // It needs to stick to 64-wesolowski limit.
            // In some cases, the last 2^16 segment might be slightly
            // inaccurate, so calculate the real number.
            ...
            for (int i = segment_count - 1; i >= 0 && proof_blobs < 63; i--) {
              ...
              if (max_proving_iteration % segment_size != 0) {
                ...
                while (position < done_segments[i].size() &&
                       !done_segments[i][position].is_empty) {
      ...
      for (int i = 0; i < segment_count; i++) {
        ...
        while (last_appended[i] + sg_length <= intermediates_iter) {
          ...
          Segment sg(
              ...
              /*y=*/
              weso->checkpoints[(last_appended[i] + sg_length) / (1 << 16)]);
          pending_segments[i].emplace_back(sg);
          if (!warned && pending_segments[i].size() >= kWindowSize - 2) {
            warned = true;
            std::cout
                << "Warning: VDF loop way ahead of proving loop. "
      ...
      while (!stopped) {
        ...
        if (active_provers < max_proving_threads) {
          ...
          // Otherwise, pause one already running segment, if candidate is
          // better.
          Segment worst_running;
          int worst_index;
          for (int i = 0; i < provers.size(); i++)
            if (provers[i].first->IsRunning()) {
              if (worst_running.is_empty == true ||
                  provers[i].second.IsWorseThan(worst_running)) {
                worst_running = provers[i].second;
                worst_index = i;
              }
            }
          // If the candidate is better than worst running, stop worst running
          // and spawn candidate.
        ...
        // Best provers are already running, nothing will change until a new
        // event happens.
        if (!spawn_best) {
          break;
        }
        ...
        if (!new_segment) {
          ...
          if (!stopped) {
            provers.emplace_back(std::make_pair(
                std::make_unique<InterruptableProver>(best, D, weso), best));
...
private:
  ...
  // For each segment length, remember the endpoint of the last segment marked
  // as pending.
